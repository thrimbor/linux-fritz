...
out:
	TRACE("Got %s %d, start block %lld, locked %d, error %d\n",
			cache->name, i, entry->block, entry->locked, entry->error);
	if (entry->error)
...
static int supported_squashfs_filesystem(struct squashfs_sb_info *msblk, int silent)
{
	...
	/*--- SERROR("Major = %d (build in %d) Minor = %d (build in %d)\n", ---*/
	/*--- sblk->s_major, SQUASHFS_MAJOR, sblk->s_minor, SQUASHFS_MINOR); ---*/
...
static int squashfs_fill_super(struct super_block *s, void *data, int silent)
{
	...
	if(!supported_squashfs_filesystem(msblk, silent)) {
		WARNING("supported_squashfs_filesystem: not supported\n");
		goto failed_mount;
	}
	...
	if(sblk->bytes_used < 0 || sblk->bytes_used > i_size_read(s->s_bdev->bd_inode)) {
		WARNING("sblk->bytes_used(%Lx) < 0 || sblk->bytes_used(%Lx) > i_size_read(s->s_bdev->bd_inode)(%Lx)\n",
				sblk->bytes_used, sblk->bytes_used, i_size_read(s->s_bdev->bd_inode));
		if(sblk->bytes_used < 0 || sblk->bytes_used > i_size_read(s->s_bdev->bd_inode) + 1024) {
			goto failed_mount;
		}
	}
	...
	if (SQUASHFS_INODE_OFFSET(sblk->root_inode) > SQUASHFS_METADATA_SIZE) {
		WARNING("SQUASHFS_INODE_OFFSET(sblk->root_inode)(%x) > SQUASHFS_METADATA_SIZE(%x)\n", SQUASHFS_INODE_OFFSET(sblk->root_inode), SQUASHFS_METADATA_SIZE);
		goto failed_mount;
	}
	...
	if((sblk->s_minor == SQUASHFS_MINOR_LZMA) || (sblk->s_minor == SQUASHFS_MINOR_AES_LZMA) || (sblk->s_minor == SQUASHFS_MINOR_CBC_LZMA)) {
		printk("[squashfs] use lzma compression \n");

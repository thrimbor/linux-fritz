                                        ...
                                        enum yaffs_ecc_result ecc_result)
{
	...
	yaffs_handle_chunk_error(dev, bi, ecc_result);
				 ...
				 struct yaffs_ext_tags *tags, int use_reserver)
{
	...
	do {
		...
		if (write_ok != YAFFS_OK) {
			 ...
			 * try another chunk */
			yaffs_handle_chunk_wr_error(dev, chunk, erased_ok, tags->ecc_result);
			continue;
		}
		...
		list_for_each_safe(lh, n, &dev->obj_bucket[i].list) {
			...
			if (yaffs_has_null_parent(dev, obj)) {
				...
				hanging = 0;
                       ...
                       && (obj->variant.hardlink_variant.equiv_obj == 0)) {
                /**
                 * <hbl>
                 * Here is an open hardlink end -> not so good
                 *
                 * This yaffs version has a problem with hardlinks.
                 * It can be, that a hardlinked file will lose its source/target.
                 * So its link directs to a NULL object -> and this is NOT handled in this *** code.
                 * To clean up this 'one-sides' hardlinked file, it will be moved into the lost+found dir.
                 */
                static YCHAR obj_name[YAFFS_MAX_NAME_LENGTH + 1];

                yaffs_get_obj_name(obj, obj_name,YAFFS_MAX_NAME_LENGTH + 1);
                printk(KERN_ERR "detected hardlink with open end (inode=%u, name='%s') -> move to 'lost+found'", obj->obj_id, obj_name);
...
struct yaffs_obj *yaffs_get_equivalent_obj(struct yaffs_obj *obj)
{
	if (obj && obj->variant_type == YAFFS_OBJECT_TYPE_HARDLINK) {
        if (obj->variant.hardlink_variant.equiv_obj) {
            obj = obj->variant.hardlink_variant.equiv_obj;
            yaffs_check_obj_details_loaded(obj);
        } else {
            printk(KERN_ERR "!!!! HARDLINK-FAILURE: obj->obj_id=%u", obj->obj_id);
        }
    }
	return obj;
}

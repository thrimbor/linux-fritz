...
#include <switch_api/ifx_switch_ll.h>
#include <switch_api/VR9_switch.h>
#include <switch_api/VR9_top.h>
...
IFX_FLOW_PCE_MICROCODE pce_mc_max_ifx_tag_m = {
	...
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 40 , OUT_NONE ,		2 , INSTR , FLAG_NO , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_APP0 ,		2 , INSTR , FLAG_NO , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_NONE ,		0 , INSTR , FLAG_END , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_NONE ,		0 , INSTR , FLAG_END , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_NONE ,		0 , INSTR , FLAG_END , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_NONE ,		0 , INSTR , FLAG_END , 0),
	IFX_FLOW_PCE_MC_M(0x0000 , 0x0000 , 41 , OUT_NONE ,		0 , INSTR , FLAG_END , 0),
...
static IFX_int32_t get_tbl_index(void *pTblStart, void *pPar, IFX_uint32_t TblEntrySize,IFX_uint32_t TblEntryNum)
{
	IFX_int32_t i;
	/* search if the entry is already available and can be re-used */
	for (i = 0; i < TblEntryNum; i++) {
...
/* Static Function Declaration */
IFX_int32_t find_software_tbl_entry(IFX_PCE_HANDLE_t *pTmHandle, IFX_PCE_IP_DASA_LSB_TBL_t *pPar)
{
	return (get_tbl_index(pTmHandle->ip_dasa_lsb_tbl,pPar, sizeof(IFX_PCE_IP_DASA_LSB_TBL_t),	\
		IFX_PCE_IP_DASA_LSB_TBL_SIZE));
}

/* Static Function Declaration */
IFX_int32_t find_software_msb_tbl_entry(IFX_PCE_HANDLE_t *pTmHandle, IFX_PCE_IP_DASA_MSB_TBL_t *pPar)
{
	return (get_tbl_index(pTmHandle->ip_dasa_msb_tbl,pPar, sizeof(IFX_PCE_IP_DASA_MSB_TBL_t),	\
		IFX_PCE_IP_DASA_MSB_TBL_SIZE));
}
...
IFX_int32_t ifx_pce_tm_ip_dasa_msb_tbl_idx_delete(IFX_PCE_HANDLE_t *pTmHandle,IFX_uint32_t index)
{
	return tbl_idx_delete(pTmHandle->ip_dasa_msb_tbl_cnt, index,IFX_PCE_IP_DASA_MSB_TBL_SIZE);
}
...
IFX_return_t IFX_VR9_Switch_PCE_Micro_Code_Int(IFX_void_t *pDevCtx)
{
	...
	for (j =0; j < pEthSWDev->nPortNumber;j++) {
		ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, (VR9_FDMA_PCTRL_EN_OFFSET + (j * 0x6)),	\
                               VR9_FDMA_PCTRL_EN_SHIFT, VR9_FDMA_PCTRL_EN_SIZE, 0);
		ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, (VR9_SDMA_PCTRL_PEN_OFFSET + (j * 0x6)),	\
                               VR9_SDMA_PCTRL_PEN_SHIFT, VR9_SDMA_PCTRL_PEN_SIZE, 0);
   }
	/* Set GLOB_CTRL.SE */
	ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, (VR9_GLOB_CTRL_SE_OFFSET + 0xC40),	\
	...
	for (i = 0; i < IFX_FLOW_PCE_MICROCODE_VALUES; i++) {
		...
		ifx_ethsw_xwayflow_pce_table_write(pEthSWDev, &tbl_entry);
	}

	/* PCE parser microcode is valid */
	ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, VR9_PCE_GCTRL_0_MC_VALID_OFFSET,	\
         VR9_PCE_GCTRL_0_MC_VALID_SHIFT, VR9_PCE_GCTRL_0_MC_VALID_SIZE, 0x1);

	/* UTP: SMS01358977 - Default unknown Multicast/Unicast*/
	ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, VR9_PCE_PMAP_2_DMCPMAP_OFFSET,	\
		VR9_PCE_PMAP_2_DMCPMAP_SHIFT, VR9_PCE_PMAP_2_DMCPMAP_SIZE, 0x7F);
	ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, VR9_PCE_PMAP_3_UUCMAP_OFFSET,	\
	...
	for (j = 0; j < pEthSWDev->nPortNumber; j++) {
		ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, (VR9_BM_PCFG_CNTEN_OFFSET + (j * 0x2)),	\
			VR9_BM_PCFG_CNTEN_SHIFT, VR9_BM_PCFG_CNTEN_SIZE, 1);
	}
   
	/* The Threshold bit is set to local threshold by default */
	ifx_ethsw_ll_DirectAccessWrite(pEthSWDev, VR9_BM_QUEUE_GCTRL_GL_MOD_OFFSET,	\
...
//		dasa_msb_tbl.mask = pPar->pattern.nDstIP_Mask;/* DIP MSB Nibble Mask */
		dasa_msb_tbl.mask = (IFX_uint16_t) ((pPar->pattern.nDstIP_Mask > 16) & 0xFFFF);
		tab_index = find_software_msb_tbl_entry(&pHandle->pce_sub_tbl, &dasa_msb_tbl);
		if (tab_index < 0 ) {
			IFXOS_PRINT_INT_RAW(" ERROR: %s:%s:%d (DASA Table full ) \n", __FILE__, __FUNCTION__, __LINE__);
			return IFX_ERROR;
		}
		if (tab_index == 0xFF ) {
...
//		dasa_lsb_tbl.mask = pPar->pattern.nDstIP_Mask;;/* DIP LSB Nibble Mask */
		dasa_lsb_tbl.mask = (IFX_uint16_t) (pPar->pattern.nDstIP_Mask & 0xFFFF);
	...
	if (pPar->pattern.eDstIP_Select == IFX_FLOW_PCE_IP_V4) {
		...
		/* DIP LSB Nibble Mask */
//		dasa_lsb_tbl.mask = pPar->pattern.nDstIP_Mask;
		dasa_lsb_tbl.mask = (IFX_uint16_t) (pPar->pattern.nDstIP_Mask | 0xFF00) & 0xFFFF;
		tab_index = find_software_tbl_entry(&pHandle->pce_sub_tbl, &dasa_lsb_tbl);
		if (tab_index < 0 ) {
			IFXOS_PRINT_INT_RAW(" ERROR: %s:%s:%d (DASA Table full  ) \n", __FILE__, __FUNCTION__, __LINE__);
			return IFX_ERROR;
...
//		dasa_msb_tbl.mask = pPar->pattern.nSrcIP_Mask;/* DIP MSB Nibble Mask */
		dasa_msb_tbl.mask = (IFX_uint16_t) ((pPar->pattern.nSrcIP_Mask > 16) & 0xFFFF);
		tab_index = find_software_msb_tbl_entry(&pHandle->pce_sub_tbl, &dasa_msb_tbl);
		if (tab_index < 0 ) {
			IFXOS_PRINT_INT_RAW(" ERROR: %s:%s:%d (DASA Table full  ) \n", __FILE__, __FUNCTION__, __LINE__);
			return IFX_ERROR;
...
//		dasa_lsb_tbl.mask = pPar->pattern.nSrcIP_Mask;/* DIP LSB Nibble Mask */
		dasa_lsb_tbl.mask = (IFX_uint16_t) (pPar->pattern.nSrcIP_Mask & 0xFFFF);
		tab_index = find_software_tbl_entry(&pHandle->pce_sub_tbl, &dasa_lsb_tbl);
		if (tab_index == 0xFF ) {
			tab_index = ifx_pce_tm_ip_dasa_lsb_tbl_write(&pHandle->pce_sub_tbl, &dasa_lsb_tbl);
		}
...
//		dasa_lsb_tbl.mask = pPar->pattern.nSrcIP_Mask;
		dasa_lsb_tbl.mask = (IFX_uint16_t) (pPar->pattern.nSrcIP_Mask | 0xFF00) & 0xFFFF;
		tab_index = find_software_tbl_entry(&pHandle->pce_sub_tbl, &dasa_lsb_tbl);
		if (tab_index == 0xFF ) {
			tab_index = ifx_pce_tm_ip_dasa_lsb_tbl_write(&pHandle->pce_sub_tbl, &dasa_lsb_tbl);
		}
...
//  ifx_pce_table_init(pHandle);
   if (pHandle == IFX_NULL)
      return IFX_ERROR;
    for (i = 0; i < IFX_PCE_TBL_SIZE; i++) {
      IFX_PCE_TBL_t *pFet = &pHandle->pce_tbl[i];
      IFX_FLOW_PCE_action_t *pAct = &pHandle->pce_act[i];
        if (pHandle->pce_tbl_used[i] == 1) {
      IFXOS_PRINT_INT_RAW("Rule %d:\n",i);
      IFXOS_PRINT_INT_RAW("\tdscp\t%d\n", pFet->dscp);
      IFXOS_PRINT_INT_RAW("\tpcp\t%d\n", pFet->pcp);
      IFXOS_PRINT_INT_RAW("\tpkt_lng_idx\t%d\n", pFet->pkt_lng_idx);
      IFXOS_PRINT_INT_RAW("\tdst_mac_addr_idx\t%d\n", pFet->dst_mac_addr_idx);
      IFXOS_PRINT_INT_RAW("\tsrc_mac_addr_idx\t%d\n", pFet->src_mac_addr_idx);
      IFXOS_PRINT_INT_RAW("\tdst_appl_fld_idx\t%d\n", pFet->dst_appl_fld_idx);
      IFXOS_PRINT_INT_RAW("\tsrc_appl_fld_idx\t%d\n", pFet->src_appl_fld_idx);
      IFXOS_PRINT_INT_RAW("\tdip_msb_idx\t%d\n", pFet->dip_msb_idx);
      IFXOS_PRINT_INT_RAW("\tdip_lsb_idx\t%d\n", pFet->dip_lsb_idx);
      IFXOS_PRINT_INT_RAW("\tsip_msb_idx\t%d\n", pFet->sip_msb_idx);
      IFXOS_PRINT_INT_RAW("\tsip_lsb_idx\t%d\n", pFet->sip_lsb_idx);
      IFXOS_PRINT_INT_RAW("\tip_prot_idx\t%d\n", pFet->ip_prot_idx);
      IFXOS_PRINT_INT_RAW("\tethertype_idx\t%d\n", pFet->ethertype_idx);
      IFXOS_PRINT_INT_RAW("\tpppoe_idx\t%d\n", pFet->pppoe_idx);
      IFXOS_PRINT_INT_RAW("\tvlan_idx\t%d\n", pFet->vlan_idx);
      IFXOS_PRINT_INT_RAW("\tport_id\t%d\n", pFet->port_id);
      IFXOS_PRINT_INT_RAW("Action %d:\n",i);
      IFXOS_PRINT_INT_RAW("\teTrafficClassAction\t%d\n", pAct->eTrafficClassAction);
      IFXOS_PRINT_INT_RAW("\tnTrafficClassAlternate\t%d\n", pAct->nTrafficClassAlternate);
      IFXOS_PRINT_INT_RAW("\teSnoopingTypeAction\t%d\n", pAct->eSnoopingTypeAction);
      IFXOS_PRINT_INT_RAW("\teLearningAction\t%d\n", pAct->eLearningAction);
      IFXOS_PRINT_INT_RAW("\teIrqAction\t%d\n", pAct->eIrqAction);
      IFXOS_PRINT_INT_RAW("\teCrossStateAction\t%d\n", pAct->eCrossStateAction);
      IFXOS_PRINT_INT_RAW("\teCritFrameAction\t%d\n", pAct->eCritFrameAction);
      IFXOS_PRINT_INT_RAW("\teTimestampAction\t%d\n", pAct->eTimestampAction);
      IFXOS_PRINT_INT_RAW("\tePortMapAction\t%d\n", pAct->ePortMapAction);
      IFXOS_PRINT_INT_RAW("\tnForwardPortMap\t%d\n", pAct->nForwardPortMap);
      IFXOS_PRINT_INT_RAW("\tbRemarkAction\t%d\n", pAct->bRemarkAction);
      IFXOS_PRINT_INT_RAW("\tbRemarkPCP\t%d\n", pAct->bRemarkPCP);
      IFXOS_PRINT_INT_RAW("\tbRemarkDSCP\t%d\n", pAct->bRemarkDSCP);
      IFXOS_PRINT_INT_RAW("\tbRemarkClass\t%d\n", pAct->bRemarkClass);
      IFXOS_PRINT_INT_RAW("\teMeterAction\t%d\n", pAct->eMeterAction);
      IFXOS_PRINT_INT_RAW("\tnMeterId\t%d\n", pAct->nMeterId);
      IFXOS_PRINT_INT_RAW("\tbRMON_Action\t%d\n", pAct->bRMON_Action);
      IFXOS_PRINT_INT_RAW("\tnRMON_Id\t%d\n", pAct->nRMON_Id);
      IFXOS_PRINT_INT_RAW("\teVLAN_Action\t%d\n", pAct->eVLAN_Action);
      IFXOS_PRINT_INT_RAW("\tnVLAN_Id\t%d\n", pAct->nVLAN_Id);
      IFXOS_PRINT_INT_RAW("\teVLAN_CrossAction\t%d\n", pAct->eVLAN_CrossAction);
      IFXOS_PRINT_INT_RAW("\n");
      }
   }
 //  return ifx_pce_tm_print_tables(&pHandle->pce_sub_tbl);
   return IFX_SUCCESS;
}
...
{
   IFX_int32_t i;
   IFX_int32_t table_printed = 0;
   table_printed = 0;
    for (i = 0; i < IFX_PCE_PKG_LNG_TBL_SIZE; i++) {
        if (pTmHandle->pkg_lng_tbl_cnt[i]) {
            if (table_printed == 0) {
            table_printed = 1;
            IFXOS_PRINT_INT_RAW("\nContent Packet Length Table\n===========================\n");
         }
            IFXOS_PRINT_INT_RAW("\t%d: Pkt lng %d, Pkt lng range %d\n", i,  \
                pTmHandle->pkg_lng_tbl[i].pkg_lng, pTmHandle->pkg_lng_tbl[i].pkg_lng_rng);
        }
    }

    table_printed = 0;
    for (i = 0; i < IFX_PCE_DASA_MAC_TBL_SIZE; i++) {
        if (pTmHandle->dasa_mac_tbl_cnt[i]) {
            if (table_printed == 0) {
            table_printed = 1;
            IFXOS_PRINT_INT_RAW("\nContent DASA MAC Table\n======================\n");
         }
            IFXOS_PRINT_INT_RAW("\t%d: MAC 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X, Mask 0x%04X\n", i,    \
                pTmHandle->dasa_mac_tbl[i].mac[0], pTmHandle->dasa_mac_tbl[i].mac[1],   \
                pTmHandle->dasa_mac_tbl[i].mac[2],pTmHandle->dasa_mac_tbl[i].mac[3],    \
                pTmHandle->dasa_mac_tbl[i].mac[4], pTmHandle->dasa_mac_tbl[i].mac[5],   \
            pTmHandle->dasa_mac_tbl[i].mac_mask);
      }
   }
    table_printed = 0;
    for (i = 0; i < IFX_PCE_APPL_TBL_SIZE; i++) {
        if (pTmHandle->appl_tbl_cnt[i]) {
            if (table_printed == 0) {
            table_printed = 1;
            IFXOS_PRINT_INT_RAW("\nContent Application Table\n=========================\n");
         }
            IFXOS_PRINT_INT_RAW("\t%d: Appl data 0x%04X, Mask/Range 0x%04X\n", i,   \
                pTmHandle->appl_tbl[i].appl_data, pTmHandle->appl_tbl[i].mask_range);
        }

      }
    table_printed = 0;
    for (i = 0; i < IFX_PCE_IP_DASA_MSB_TBL_SIZE; i++) {
        if (pTmHandle->ip_dasa_msb_tbl_cnt[i]) {
            if (table_printed == 0) {
            table_printed = 1;
            IFXOS_PRINT_INT_RAW("\nContent IP DASA MSB Table\n=========================\n");
         }
            IFXOS_PRINT_INT_RAW("\t%d: IP 15:8 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, mask 0x%04X\n", i,   \
                pTmHandle->ip_dasa_msb_tbl[i].ip_msb[0], pTmHandle->ip_dasa_msb_tbl[i].ip_msb[1],   \
                pTmHandle->ip_dasa_msb_tbl[i].ip_msb[2], pTmHandle->ip_dasa_msb_tbl[i].ip_msb[3],   \
                pTmHandle->ip_dasa_msb_tbl[i].ip_msb[4], pTmHandle->ip_dasa_msb_tbl[i].ip_msb[5],   \
            pTmHandle->ip_dasa_msb_tbl[i].mask);
      }

    }

    table_printed = 0;
    for (i = 0; i < IFX_PCE_IP_DASA_LSB_TBL_SIZE; i++) {
        if (pTmHandle->ip_dasa_lsb_tbl_cnt[i]) {
            if (table_printed == 0) {
            table_printed = 1;
            IFXOS_PRINT_INT_RAW("\nContent IP DASA LSB Table\n=========================\n");
         }
            IFXOS_PRINT_INT_RAW("\t%d: IP 7:0 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, mask 0x%04X\n",i, \
                pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[0],pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[1],    \
                pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[2],pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[3],    \
                pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[4],pTmHandle->ip_dasa_lsb_tbl[i].ip_lsb[5],    \
            pTmHandle->ip_dasa_lsb_tbl[i].mask);
      }
   }

   table_printed = 0;
    for (i = 0; i < IFX_PCE_IP_DASA_LSB_TBL_SIZE; i++) {
        if (pTmHandle->ptcl_tbl_cnt[i]) {
            if (table_printed == 0) {
                table_printed = 1;
                IFXOS_PRINT_INT_RAW("\nContent Protocl Table\n=====================\n");
            }
            IFXOS_PRINT_INT_RAW("\t%d: Eth 0x%04X, Eth Mask 0x%04X, Prot 0x%02X, Prot Flags 0x%02X, \
                Prot Mask %d, Prot Flag Mask %d\n", i,pTmHandle->ptcl_tbl[i].key.ethertype, \
                pTmHandle->ptcl_tbl[i].mask.ethertype_mask, pTmHandle->ptcl_tbl[i].key.prot.protocol,   \
                pTmHandle->ptcl_tbl[i].key.prot.protocol_flags, pTmHandle->ptcl_tbl[i].mask.prot.protocol_mask, \
            pTmHandle->ptcl_tbl[i].mask.prot.protocol_flag_mask);
      }
   }
    table_printed = 0;
    for (i = 0; i < IFX_PCE_PPPOE_TBL_SIZE; i++) {
        if (pTmHandle->pppoe_tbl_cnt[i]) {
            if (table_printed == 0) {
                table_printed = 1;
                IFXOS_PRINT_INT_RAW("\nContent Protocl Table\n=====================\n");
            }
            IFXOS_PRINT_INT_RAW("\t%d: PPPoE sess ID 0x%04X\n",  i,pTmHandle->pppoe_tbl[i].sess_id);
        }
    }
    table_printed = 0;
    for (i = 0; i < IFX_PCE_VLAN_ACT_TBL_SIZE; i++) {
        if (pTmHandle->vlan_act_tbl_cnt[i]) {
            if (table_printed == 0) {
                table_printed = 1;
                IFXOS_PRINT_INT_RAW("\nContent Protocl Table\n=====================\n");
            }
#if 0
            IFXOS_PRINT_INT_RAW("\t%d: VLAN ID 0x%04X, Repl. Flag %d, FID %d\n",i,  \
            pTmHandle->vlan_act_tbl[i].vid, pTmHandle->vlan_act_tbl[i].repl_flg, pTmHandle->vlan_act_tbl[i].fid);
#endif
        }
    }
    return 0;

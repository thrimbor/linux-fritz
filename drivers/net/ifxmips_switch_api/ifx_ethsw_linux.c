...
#include <linux/fs.h>
#include <switch_api/ifx_ethsw_linux.h>
	...
	while (pLlTable != IFX_NULL) {
		if (_IOC_TYPE(command) == pLlTable->nType) {
            ...
            IFXOS_PRINT_INT_RAW("pDrv=%p, pDrv->pLlHandle=%p\n", pDrv, pDrv->pLlHandle);
	   			...
	   			/* ioctl functions are replaced by a common function to send msg to  mailbox,
	    			 * It take the ioctl command as the first argument.	 */
				retvalue = fkt((void *)command, (IFX_uint32_t)pDrv->paramBuffer);

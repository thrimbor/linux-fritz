...
struct switch_command commands[] =
{
  ...
  { "CfgSet", "llllls", IFX_ETHSW_CFG_SET,
    //"<Enable> <Mac Table Aging Timer> <VLAN Aware> <Packet Length> <Pause Mac Mode> <Pause Mac Address>",
    "<Mac Table Aging Timer> <VLAN Aware> <Packet Length> <Learning Limit Action (0=drop)> <Pause Mac Mode> <Pause Mac Address>",
...
/*    "\ttraffic class:              (0..3)\n" */
  ...
  { "MulticastTableEntryAdd", "llssl", IFX_ETHSW_MULTICAST_TABLE_ENTRY_ADD,
    ...
    "\tGDA:                        Group Destination IP address (xxx.xxx.xxx.xxx)\n"
    "\tGSA:                        Group Source IP address (xxx.xxx.xxx.xxx)\n"
...
#ifdef AR9
static char switch_dev[]="/dev/switch/1";
#endif
...
#ifdef VR9
static char switch_dev[]="/dev/switch/0";
#endif
	...
	for (;;) {
		...
		if (portMemberRead.nVId)
			printf("%20d | %20d | %s\n", portMemberRead.nVId, portMemberRead.nPortId , 	\
				(portMemberRead.nTagId > 0)?"TRUE":"FALSE");
		memset(&portMemberRead, 0x00, sizeof(portMemberRead));
...
#if 0
        printf("%03d.%03d.%03d.%03d  |",
        p->multicast_TableEntryRead.uIP_Gda.nIPv4         & 0xFF,
        (p->multicast_TableEntryRead.uIP_Gda.nIPv4 >> 8)  & 0xFF,
        (p->multicast_TableEntryRead.uIP_Gda.nIPv4 >> 16) & 0xFF,
        ((p->multicast_TableEntryRead.uIP_Gda.nIPv4 >> 24) & 0xFF));

            printf("%03d.%03d.%03d.%03d  |",
            p->multicast_TableEntryRead.uIP_Gsa.nIPv4         & 0xFF,
            (p->multicast_TableEntryRead.uIP_Gsa.nIPv4 >> 8)  & 0xFF,
            (p->multicast_TableEntryRead.uIP_Gsa.nIPv4 >> 16) & 0xFF,
            ((p->multicast_TableEntryRead.uIP_Gsa.nIPv4 >> 24) & 0xFF));
#endif
}
...
{
   printf("  %d   |", p->multicast_TableEntryRead.nPortId);
   // GDA field
   ...
   for ( i = 0; i < 12; i++)
   {
      if (((p->multicast_TableEntryRead.nPortId >> i) & 0x1) == 1)
      {
         printf("  %d   |", i);
         // GDA field
         if (p->multicast_TableEntryRead.eIPVersion == IFX_ETHSW_IP_SELECT_IPV4)
...
/*           x.MAC_tableAdd.nTrafficClass = cmd_arg[4];*/
  ...
  //            if( convert_ip_adr_str((char*)cmd_arg[2], &x.multicast_TableEntryRemove.uIP_Gda.nIPv6[0]) != IFX_SUCCESS)
  //                return -1;
           } else {
            printf ("The input IP Version is not accept.\n");
           }
  ...
  //            if( convert_ip_adr_str((char*)cmd_arg[3], &x.multicast_TableEntryRemove.uIP_Gsa.nIPv6[0]) != IFX_SUCCESS)
  //                return -1;
           } else {
            printf ("The input IP Version is not accept.\n");
           }
       ...
       case IFX_ETHSW_MDIO_CFG_SET:
               ...
               x.mdio_cfg.eMDIO_Speed = cmd_arg[1];
               ioctl_params = (void *)&x.mdio_cfg;
               break;
...
*/
       ...
       case IFX_FLOW_PCE_RULE_WRITE:
           ...
           x.pce_rule.pattern.nDstIP_MsbMask = cmd_arg[27];
           ...
           x.pce_rule.pattern.nSrcIP_MsbMask = cmd_arg[30];
       ...
       case IFX_ETHSW_CFG_SET:
           ...
           x.cfg_Data.bLearningLimitAction = cmd_arg[3];
           x.cfg_Data.bPauseMAC_ModeSrc = cmd_arg[4];
           if( convert_mac_adr_str((char*)cmd_arg[5], &x.cfg_Data.nPauseMAC_Src[0]) != IFX_SUCCESS)
               return -1;
...
/*--- #ifdef VERBOSE_PRINT ---*/
         ...
         case IFX_ETHSW_MDIO_CFG_GET:
                ...
                printf("\tClock Speed= %d\n", x.mdio_cfg.eMDIO_Speed);
#else
                printf("bMDIO_Enable=%d ", x.mdio_cfg.bMDIO_Enable);
                printf("eMDIO_Speed=%d ", x.mdio_cfg.eMDIO_Speed);
#endif
           break;
         case IFX_ETHSW_VLAN_ID_GET:
#ifdef VERBOSE_PRINT
 ...
 //               printf("eProfile= 0x%x ", x.qos_wredqueuecfg.eProfile);
       ...
       case IFX_ETHSW_CPU_PORT_EXTEND_CFG_GET:
         ...
         case IFX_FLOW_PCE_RULE_READ:
                ...
                printf("\tDIP Nibble Mask                      = 0x%x\n", x.pce_rule.pattern.nDstIP_MsbMask);
                ...
                printf("\tSIP Nibble Mask                      = 0x%x\n", x.pce_rule.pattern.nSrcIP_MsbMask);
                ...
                printf("\tnDstIP_MsbMask=0x%x", x.pce_rule.pattern.nDstIP_MsbMask);
                printf("\teSrcIP_Select=0x%x", x.pce_rule.pattern.eSrcIP_Select);
                printf("\tnSrcIP=0x%x", x.pce_rule.pattern.nSrcIP);
                printf("\tnSrcIP_MsbMask=0x%x", x.pce_rule.pattern.nSrcIP_MsbMask);

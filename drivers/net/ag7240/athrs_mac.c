...
#include "../avm_cpmac/cpmac_virian_if.h"
...
#if 0   
    if (is_ar934x()) {
        mask = (ATHR_RESET_GE0_PHY | ATHR_RESET_GE1_PHY);
...
#   endif /*--- #if defined(CONFIG_AVM_CPMAC) ---*/
    ...
    if (mac_has_flag(mac, ETH_LINK_INTERRUPT)) {
        spin_lock_irqsave(&mac->mac_lock, flags);
        printk("[%s] setup MACLINK-IRQ\n", __FUNCTION__);
        if (mac->mac_unit == 0) {
            printk("[%s] setup S16 MACLINK-IRQ mac_unit %d %d\n", __FUNCTION__, mac->mac_unit, mac->mac_linkirq);
            st = request_irq(mac->mac_linkirq, athr_gmac_link_intr, IRQF_DISABLED | IRQF_TRIGGER_LOW, "gmac s16 link intr", dev);
        } else {
            /*--- printk("[%s] setup MACLINK-IRQ mac_unit %d\n", __FUNCTION__, mac->mac_unit); ---*/
            st = request_irq(ATHR_IRQ_ENET_LINK, athr_gmac_link_intr, IRQF_DISABLED | IRQF_SAMPLE_RANDOM, "gmac link intr", dev);
            athr_gmacs[mac->mac_unit]->mac_linkirq = ATHR_IRQ_ENET_LINK;
        }
        if (st < 0) {
            printk(MODULE_NAME ": request irq %d failed %d\n" , ATHR_IRQ_ENET_LINK, st);
            free_irq(mac->mac_irq, dev); 
            athr_gmacs[mac->mac_unit]->mac_linkirq = 0;
        }
        spin_unlock_irqrestore(&mac->mac_lock, flags);
    }
...
athr_gmac_stop(struct net_device *dev)
{
    ...
    if (mac_has_flag(mac,ETH_LINK_INTERRUPT)) {
        if (athr_gmacs[mac->mac_unit]->mac_linkirq) {
            free_irq(ATHR_IRQ_ENET_LINK, athr_gmacs[mac->mac_unit]);
        }
    }
...
athr_gmac_mii_setup(athr_gmac_t *mac)
{
    ...
    printk("[%s] WASP unit %d ", __FUNCTION__, mac->mac_unit);

    /*
     * set the mii if type - NB reg not in the gigE space
     */
    if (is_ar934x() && is_f2e() && mac->mac_unit == 0) {
        printk("WASP ----> F2e PHY\n");
        ...
        */
        ...
        athr_reg_rmw_set(ATHR_SWITCH_CLK_SPARE, (1 << 6));
    ...
    if (is_ar934x() && is_s27() && mac->mac_unit == 1) {
        if (!is_emu()) {
...
athr_gmac_hw_setup(athr_gmac_t *mac)
{
    ...
    athr_gmac_reg_wr(mac, ATHR_GMAC_DMA_RX_STATUS, athr_gmac_reg_rd(mac, ATHR_GMAC_DMA_RX_STATUS));
    athr_gmac_reg_wr(mac, ATHR_GMAC_CFG1, (ATHR_GMAC_CFG1_RX_EN | ATHR_GMAC_CFG1_TX_EN));

    if ((mac->mac_unit == 1))
       athr_gmac_reg_rmw_set(mac, ATHR_GMAC_CFG2, 
               (ATHR_GMAC_CFG2_PAD_CRC_EN | ATHR_GMAC_CFG2_LEN_CHECK | ATHR_GMAC_CFG2_IF_1000)); 

    if (mac->mac_unit == 0)
       athr_gmac_reg_rmw_set(mac, ATHR_GMAC_CFG2, 
               (ATHR_GMAC_CFG2_PAD_CRC_EN | ATHR_GMAC_CFG2_LEN_CHECK | ATHR_GMAC_CFG2_IF_10_100));
    ...
    else if ((mac->mac_unit == 0 || mac->mac_unit == 1) && (is_ar7242() || is_s16() || is_s17() || mac_has_flag(mac,ATHR_JUMBO_FR))) {
       athr_gmac_reg_wr(mac, ATHR_GMAC_FIFO_CFG_5, 0xe6be2);
    }
    else if (is_f2e()) {
       athr_gmac_reg_wr(mac, ATHR_GMAC_FIFO_CFG_5, 0xbefff);
    ...
    if (mac_has_flag(mac,ATHR_JUMBO_FR)) {
       athr_gmac_reg_rmw_set(mac, ATHR_GMAC_CFG2, ATHR_GMAC_CFG2_HUGE_FRAME_EN);
...
athr_gmac_hw_stop(athr_gmac_t *mac)
{
    ...
    */
     ...
     */
   if(!is_ar9330_emu() && ((mac->mac_unit == 1) || is_ar7241() || is_ar7242() || is_emu() || is_ar934x())) {
...
athr_gmac_set_mac_from_link(athr_gmac_t *mac, athr_phy_speed_t speed, int fdx)
{
    ...
    switch (speed) {
        case ATHR_PHY_SPEED_1000T: 
            ...
            if (is_ar934x() && (mac->mac_unit == 0) && (is_s16() || is_s17()))
                athr_reg_wr(ATHR_GMAC_XMII_CONFIG, (1<<26)|(1<<25));

            if (is_ar934x() && mac->mac_unit == 0 && is_f1e() )
                athr_reg_wr(ATHR_GMAC_XMII_CONFIG,0x0e000000);

            if (is_ar9344() && (mac->mac_unit == 0) && is_f2e() ) {
                athr_reg_wr(ATHR_GMAC_XMII_CONFIG, (1<<25));
            }
        ...
        case ATHR_PHY_SPEED_100T:
            ...
            if (is_emu() && is_f1e()) {
                athr_reg_wr(ATHR_GPIO_FUNCTIONS,0x2);
            ...
            if ((is_ar7242() || is_ar934x()) && (mac->mac_unit == 0) && 
                    (is_s16() || is_s17() || is_f1e() || is_11g() || is_f2e())) {
...
athr_gmac_fast_reset(athr_gmac_t *mac, athr_gmac_desc_t *ds, int ac)
{
    ...
    if (ds){ 
        athr_gmac_reg_wr(mac,ATHR_GMAC_DMA_TX_DESC, (athr_gmac_desc_dma_addr(r,ds)));
    } else {
...
athr_gmac_get_link_st(athr_gmac_t *mac, int *link, int *fdx, athr_phy_speed_t *speed,int phyUnit)
{
    ...
    if(phy) {
#if HYBRID_LINK_CHANGE_EVENT
        {
            ...
            int up = 0;
            int eth01flag = 0;
            *link = phy->is_up(mac->mac_unit, &down, &up, &eth01flag);
            return ((eth01flag << 16) | ((down & 0x1f) << 8) | (up & 0x1f));
        }
#else
 ...
 */
int
athr_gmac_check_link(athr_gmac_t *mac,int phyUnit)
{
    ...
    rc = athr_gmac_get_link_st(mac, &linkup, &fdx, &speed, phyUnit);

    if(phy->stab_wr) 
        phy->stab_wr(phyUnit,linkup,speed);
    ...
    if (!linkup)
    {
        if (carrier)
        {
            ...
            if (mac_has_flag(mac,ETH_SOFT_LED)) {
                 ...
                 */
                if (is_ar933x()) { 
                    athr_reg_wr(ATHR_GPIO_FUNCTIONS, (athr_reg_rd(ATHR_GPIO_FUNCTIONS)& (~(1 << (phyUnit + 3)))));
                }
            }
        }
        goto done;
...
#if defined(CONFIG_AVM_CPMAC)
    cpmac_interface_port_update((unsigned char) mac->mac_unit, linkup,
                                (speed == ATHR_PHY_SPEED_1000T) ? avm_event_ethernet_speed_1G 
                                : ((speed == ATHR_PHY_SPEED_100T ) ? avm_event_ethernet_speed_100M 
...
athr_gmac_mii_read(int unit, uint32_t phy_addr, uint8_t reg)
{
    ...
    /*--- printk(KERN_ERR "[%s/%d] unit 0x%x phy_addr 0x%x reg_addr 0x%x\n", __FUNCTION__, __LINE__, unit, phy_addr, reg); ---*/
    ...
    if (ii == 0)
        printk(KERN_ERR "ERROR:%s:%d transaction failed\n",__func__,__LINE__);
...
athr_gmac_mii_write(int unit, uint32_t phy_addr, uint8_t reg, uint16_t data)
{
    ...
    /*--- printk(KERN_ERR "[%s/%d] unit 0x%x phy_addr 0x%x reg_addr 0x%x value 0x%x\n", __FUNCTION__, __LINE__, unit, phy_addr, reg, data); ---*/
    if(is_ar9330_emu())
        mac = athr_gmac_unit2mac(0);
    ...
    if (ii == 0)
        printk(KERN_ERR "ERROR:%s transaction failed\n",__FUNCTION__);
...
athr_gmac_link_intr(ATHR_MAC_ISR_ARGS)
{
    ...
    return mac->phy->link_isr((void *)mac);
}
...
#       if defined(CONFIG_AVM_CPMAC) && CONFIG_AVM_CPMAC
        if(!IS_ERR(&cpmac_virian_if_rx) && &cpmac_virian_if_rx) {
            cpmac_virian_if_rx(skb);
        } else
            netif_receive_skb(skb);
#       else /*--- #if defined(CONFIG_AVM_CPMAC) && CONFIG_AVM_CPMAC ---*/
        netif_receive_skb(skb);
#       endif /*--- #else ---*/ /*--- #if defined(CONFIG_AVM_CPMAC) && CONFIG_AVM_CPMAC ---*/
...
static struct sk_buff * athr_gmac_buffer_alloc(void) {
     ...
     */
#if 0
    printk(KERN_ERR "[%s] skb 0x%p skb->data 0x%p skb->end 0x%p skb_shinfo(skb) 0x%p = 0x%08x 0x%08x\n", 
            __FUNCTION__, skb, skb->data, skb->end, 
            skb_shinfo(skb),
...
{
    /*--- printk(KERN_ERR "[%s] skb 0x%x skb->data 0x%x\n", __FUNCTION__, skb, skb->data); ---*/
    if (in_irq())
...
static void
athr_gmac_ring_release(athr_gmac_t *mac __attribute__((unused)), athr_gmac_ring_t  *r)
{
    int i;
...
static void athr_gmac_get_default_macaddr(athr_gmac_t *mac, u8 *mac_addr) {
#if defined(CONFIG_AG7100_MAC_LOCATION) || defined(CONFIG_AG7240_MAC_LOCATION)
    ...
    if (!is_emu() && !is_ar9330_emu()) {
        ...
        if(eep_mac_addr[0] == 0x00 && eep_mac_addr[1] == 0x03 && eep_mac_addr[2] == 0x7f) {
            memcpy(mac_addr, eep_mac_addr, 6);
            printk(KERN_ERR "[%s]:%d WARNING: No valid MAC for this device\n", __FUNCTION__, __LINE__);
        } 
#endif
        switch( mac->mac_unit) {
            case 0: 
    ...
    printk(KERN_ERR "[%s] Setup Mac Addr for Device(%d): %02x:%02x:%02x:%02x:%02x:%02x \n",
            __FUNCTION__, mac->mac_unit,
...
athr_gmac_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    /*--- printk(KERN_ERR "[%s/%d] dev->name %s mac->name %s\n", __FUNCTION__, __LINE__, dev->name, mac->name); ---*/
    ...
    *athr_gmac_get_stats(struct net_device *dev)
{
    ...
    athr_gmac_t *mac = ATHR_MAC_PRIV(dev);
    int carrier = netif_carrier_ok(dev);
    /*--- printk(KERN_ERR "[%s/%d] dev->name %s mac->name %s carrier %s\n", ---*/ 
            /*--- __FUNCTION__, __LINE__, dev->name, mac->name, carrier ? "UP" : "DOWN"); ---*/

    if (mac->mac_speed == ATHR_PHY_SPEED_UNKNOWN) {
        /*--- printk(KERN_ERR "[%s/%d] ATHR_PHY_SPEED_UNKNOWN done\n", __FUNCTION__, __LINE__); ---*/
     ...
     */
    /*--- printk(KERN_ERR "[%s/%d] phy in reset %s carrier %s\n", __FUNCTION__, __LINE__, ---*/ 
            /*--- phy_in_reset ? "TRUE" : "FALSE", ---*/
            /*--- carrier ? "UP" : "DOWN"); ---*/
...
athr_gmac_init(void)
{
    ...
    */
    printk(KERN_ERR "[%s] ATHR_GMAC_NMACS %d\n", __FUNCTION__, ATHR_GMAC_NMACS);
    ...
    for(i = 0; i < ATHR_GMAC_NMACS; i++)
    {
        ...
        if (athr_gmac_phy_attach(mac,mac->mac_unit)) {    /*--- hier schon den PHY & Flags auswählen ---*/
            printk(KERN_ERR MODULE_NAME "NO PHY on mac %d found\n", mac->mac_unit);
            continue;
        }
        ...
        athr_gmac_reg_rmw_set(mac, ATHR_GMAC_CFG1, ATHR_GMAC_CFG1_SOFT_RST | ATHR_GMAC_CFG1_RX_RST | ATHR_GMAC_CFG1_TX_RST);
...
athr_gmac_cleanup(void)
{
    ...
    for(i = 0; i < ATHR_GMAC_NMACS; i++)
    {
        ...
        if (mac_has_flag(athr_gmacs[1],ETH_SWONLY_MODE) && i == 0) {
            continue;
        }

...
struct ag7240_mac {
    ...
    ag7240_phy_duplex_t     mac_fdx;
    ag7240_phy_link_t       mac_link;
    ...
    /*--- phy ---*/
    uint8_t                 phy_addr;
    uint8_t                 phy_in_reset;
    uint16_t               (*read_phy_int_status)(struct ag7240_mac *mac);
    uint16_t               (*read_phy_link_status)(struct ag7240_mac *mac);
    uint32_t               (*config_phy)(struct ag7240_mac *mac, uint32_t speed, uint32_t duplex);
    void                   (*setup_int)(struct ag7240_mac *mac, uint32_t on_off);
    uint32_t               (*init_phy)(struct ag7240_mac *mac);
};

typedef struct ag7240_mac ag7240_mac_t;
...
/*----------------------------------------------------------------------*\
 * Unsere Hardware ignoriert MAX_FRAME_SIZE Register und
 * schreibt immer 1568 Byte, somit ist folgendes Padding AG7242_RX_HW_WORKAROUND_PAD 
 * notwendig damit die Shared-Data-Bloecke am Frame-Ende nicht ueber-
 * schrieben werden
\*----------------------------------------------------------------------*/
#define AG7242_RX_HW_WORKAROUND_PAD 32
...
#define ag7240_mac_irq(_no)     (_no) ? ATH_CPU_IRQ_GE1 : ATH_CPU_IRQ_GE0

#define ag7240_reset_mask(_no) (_no) ? (ATH_RESET_GE1_MAC)   \
                                     : (ATH_RESET_GE0_MAC)  
...
#define ag7240_reg_rd(_mac, _reg)                                       \
            (ath_reg_rd((_mac)->mac_base + (_reg)))

#define ag7240_reg_wr(_mac, _reg, _val)                                 \
                ath_reg_wr((_mac)->mac_base + (_reg), (_val));

/*
 * The no flush version
 */
#define ag7240_reg_wr_nf(_mac, _reg, _val)                             \
                ath_reg_wr_nf((_mac)->mac_base + (_reg), (_val));

#define ag7240_reg_rmw_set(_mac, _reg, _mask)                           \
                 ath_reg_rmw_set((_mac)->mac_base + (_reg), (_mask));

#define ag7240_reg_rmw_clear(_mac, _reg, _mask)                          \
                 ath_reg_rmw_clear((_mac)->mac_base + (_reg), (_mask));
...
#define ag7240_intr_disable_recv(_mac)                                      \
    ag7240_reg_rmw_clear(mac, AG7240_DMA_INTR_MASK, (AG7240_INTR_RX ));
...
void ag7240_reg_dev(ag7240_mac_t **mac);

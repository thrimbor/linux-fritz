...
#endif /*CONFIG_AR9*/
#include "avm_cpmac/cpmac_ar9_if.h"

#define IFX_DRV_MODULE_NAME             "ifxmips_3port_sw"
#define IFX_DRV_MODULE_VERSION          "1.0.7"
static char version[] = IFX_DRV_MODULE_NAME ".c:v" IFX_DRV_MODULE_VERSION " \n";
...
static const struct net_device_ops netdev_ops = {
    ...
    .ndo_open            = ifx_eth_open,
    .ndo_stop            = ifx_eth_close,
    .ndo_start_xmit      = ifx_start_xmit,
    .ndo_do_ioctl        = ifx_eth_ioctl,
    .ndo_get_stats       = ifx_get_stats,
    .ndo_change_mtu      = ifx_change_mtu,
    .ndo_set_mac_address = ifx_set_mac_address,
    .ndo_tx_timeout      = ifx_tx_timeout,
//    .ndo_set_rx_mode     = ifx_set_rx_mode,
};

/*--- #define PRINT_AR9_DEBUG(args...) printk(args) ---*/
#define PRINT_AR9_DEBUG(args...) // leer
...
{
    IFX_switch_priv_t *priv = netdev_priv(dev);
...
/*Reset GPHY, which is connected to extern GPHY chip*/
    int dev_index = (!strcmp(dev->name, "eth0") ? 0 : 1);
    if (dev_index) {
        ar9_setup_ext_phy_reset(GPIO_RESET_GPHY0, 1);
        reset_ext_gphy(GPIO_RESET_GPHY0);
#if defined( CONFIG_IFX_MAC1) && CONFIG_IFX_MAC1
        ar9_setup_ext_phy_reset(GPIO_RESET_GPHY1, 1);
        reset_ext_gphy(GPIO_RESET_GPHY1);
#endif
    }
...
static int ifx_eth_open(struct net_device *dev)
{
    ...
    PRINT_AR9_DEBUG (KERN_ERR "[%s/%d] dev=%s\n", __FUNCTION__, __LINE__, dev->name);
    ...
    if(!IS_ERR(&cpmac_ar9_open) && &cpmac_ar9_open) {
        //hbl: cpmac_ar9_open(dev);
        cpmac_ar9_open();
    }
    ...
    if(index == 0) {
        enable_dma_channel();
        PRINT_AR9_DEBUG (KERN_ERR "[%s/%d] dma enabled\n", __FUNCTION__, __LINE__);
    }
    index++;
...
static void ifx_eth_rx(struct net_device *dev, int len,struct sk_buff* skb)
{
    ...
    if(!IS_ERR(&cpmac_ar9_if_rx) && &cpmac_ar9_if_rx) {
        cpmac_ar9_if_rx(skb);
    } else {
#       ifdef CONFIG_NAPI_ENABLED 
        netif_receive_skb(skb);
#       else /*--- #ifdef CONFIG_NAPI_ENABLED ---*/ 
        netif_rx(skb);
#       endif /*--- #else ---*/ /*--- #ifdef CONFIG_NAPI_ENABLED ---*/ 
    }
}
...
#ifdef DUMP_PACKET
    dump_buffer(__FUNCTION__,skb->data, len);
#endif
    if ((len >= 0x600) || (len < 64) ) {
...
static int ifx_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (skb ==  NULL) {
        printk(KERN_ERR "%s[%d]: Empty skb given!\n", __func__,__LINE__);
        return -EFAULT;
    }
  ...
  /*  IFX_switch_priv_t *priv = netdev_priv(dev); */
    ...
    switch(cmd){
        ...
        case AVM_CPMAC_IOCTL_GENERIC:
            if(!IS_ERR(&cpmac_ar9_ioctl) && &cpmac_ar9_ioctl) {
                err = cpmac_ar9_ioctl(dev, ifr, cmd);
            }
            break;
        default:
            err = -EINVAL;
...
static int switch_poll(struct napi_struct *napi, int budget)
{
#if 0
    ...
    *budget -= work_done;
    ...
    return ret;
#endif
	IFX_switch_priv_t *priv = container_of(napi, IFX_switch_priv_t, napi);
	struct net_device *dev = (struct net_device*)priv->owner;
    struct dma_device_info* dma_dev = g_dma_device;
    int work_done = 0;

    if (netif_running(dev)) {
        dma_device_poll(dma_dev, budget, &work_done);
    }
...
static void eth_activate_poll(struct dma_device_info* dma_dev)
{
    ...
    for(i=0; i < NUM_ETH_INF ; i++) {
        ...
        if ( napi_schedule_prep(&priv->napi) ) {
            ...
            PRINT_AR9_DEBUG (KERN_ERR "[%s/%d] successful\n", __FUNCTION__, __LINE__);
        }
        else {
            PRINT_AR9_DEBUG (KERN_ERR "[%s/%d] not successful\n", __FUNCTION__, __LINE__);
        }
    }
}
...
static void eth_inactivate_poll(struct dma_device_info* dma_dev)
{
    ...
    for(i=0;i<dma_dev->num_rx_chan;i++) {
        if((dma_dev->rx_chan[i])->control==IFX_DMA_CH_ON) {
            dma_dev->rx_chan[i]->enable_irq(dma_dev->rx_chan[i]);
            PRINT_AR9_DEBUG (KERN_ERR "[%s/%d] dma irq enabled\n", __FUNCTION__, __LINE__);
        }
    }
#endif
}
...
int ifx_eth_module_init (void)
{
    ...
    for (i=0; i< NUM_ETH_INF ; i++) {
        ...
        ifx_eth_dev[i] = alloc_netdev(sizeof(IFX_switch_priv_t), "cpmac%d", ether_setup);
        if (!ifx_eth_dev[i]) {
            printk(KERN_ERR "%s[%d]: no memory for ifx_eth_dev!!!\n", __func__,__LINE__);
            err = -ENOMEM;
            goto err_out_free_res;
        }
        ifx_eth_dev[i]->netdev_ops = &netdev_ops;
        priv = netdev_priv(ifx_eth_dev[i]);
        priv->owner = ifx_eth_dev[i];
        priv->phy_addr = 0x00;
    ...
    /* By default, auto-negotiate PAUSE. */
        ...
        priv->napi.state = 0;
        netif_napi_add(ifx_eth_dev[i], &priv->napi, switch_poll, TANTOS_NAPI_WEIGHT);
        if ( (err = register_netdev(ifx_eth_dev[i]))) {
            printk(KERN_ERR "%s[%d]: Register with network device failed!!!\n", __func__,__LINE__);
            goto err_out_free_res;
        }
        if(!IS_ERR(&cpmac_ar9_set_dev) && &cpmac_ar9_set_dev) {
            cpmac_ar9_set_dev(ifx_eth_dev[i]);
        }
    }

...
static int proc_read_vectoring(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;

    len += sprintf(page + len, "mei_dsm_cb_func_hook = %p\n", mei_dsm_cb_func_hook);
    len += sprintf(page + len, "g_ebr_block = %p\n", g_ebr_block);
...
static int proc_write_vectoring(struct file *file, const char *buf, unsigned long count, void *data)
{
    char *p;
    int len;
    char local_buf[1024];
...
//    g_proc_dir = proc_mkdir("eth", NULL);
    ...
    res = create_proc_entry("eth/vectoring",
                            0,
                            g_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_vectoring;
        res->write_proc = proc_write_vectoring;
    }
}

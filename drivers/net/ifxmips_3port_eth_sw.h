...
/* MII0 mode */
#if defined(CONFIG_MII0_RGMII_MODE)
#define MII0_MODE_SETUP                         RGMII_MODE
#else
#define MII0_MODE_SETUP                         RGMII_MODE_100MB
#endif

#if CONFIG_AR9_AVM_FRITZ_BOX
...
/*--- AVM HW ---*/
#define GPIO_RESET_GPHY0                    34
#define GPIO_RESET_GPHY1                    36
/** HW related configuration */
#define PHY0_ADDR                           0x0
#define PHY1_ADDR                           0x1
#define TANTOS_CHIP_ID                      0x2599
#define PHY_11G_CHIP_ID                     0x0302
#define PHY_11G_SUB_ID                      0x60D1
...
/*------------------------------------------------------------------------------------------*\
 * Setup of the reset GPIO for the external PHY
\*------------------------------------------------------------------------------------------*/
void ar9_setup_ext_phy_reset(int gpio, unsigned char en)
{
    static char enable_status[64];
    ...
    /*--- printk("[%s] GPIO %d status change %sabled -> %sabled\n", ---*/ 
               /*--- __FUNCTION__, gpio, enable_status[gpio] ? "en" : "dis", en ? "en" : "dis"); ---*/
    if(enable_status[gpio] == en) {
        return;
    }
    enable_status[gpio] = en;
    if (en) {
        ifx_gpio_pin_reserve(gpio, eth_gpio_module_id);
...
EXPORT_SYMBOL(ar9_setup_ext_phy_reset);


/*------------------------------------------------------------------------------------------*\
 * Reset of the Extern GPHY                                                                 *
\*------------------------------------------------------------------------------------------*/
void reset_ext_gphy(int gpio)
{
    /*--- printk("[%s] Reset external PHY at GPIO %d\n", __FUNCTION__, gpio); ---*/
...
#endif /*--- #if 0 ---*/ /* No external switch used */


/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void ar9_switch_write_mdio(unsigned int uPhyAddr, unsigned int uRegAddr,unsigned int data) {
    unsigned int reg;
...
EXPORT_SYMBOL(ar9_switch_write_mdio);
...
EXPORT_SYMBOL(ar9_switch_read_mdio);


/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
unsigned int ar9_switch_read(unsigned int uRegAddr) {
    return IFX_REG_R32((volatile unsigned int*) (IFX_AR9_ETH_REG_BASE + uRegAddr));
}
EXPORT_SYMBOL(ar9_switch_read);


/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void ar9_switch_write(unsigned int uRegAddr, unsigned int data) {
    IFX_REG_W32(data, (volatile unsigned int*) (IFX_AR9_ETH_REG_BASE + uRegAddr));
}
EXPORT_SYMBOL(ar9_switch_write);
...
#   if 0 /* External hardware is initializied by the CPMAC driver */
    /* Initilize the externl Tanto switch */
    if ( port == 0)
        tantos_switch_init();
#   endif /*--- #if 0 ---*/ /* External hardware is initializied by the CPMAC driver */
        
#   if 0 /* There should be no delays for AVM hardware! */
...
#endif /* SELECT_25MHZ_CLOCK_FROM_GPIO */
    /*GPIO P2.0*/
    ar9_setup_ext_phy_reset(GPIO_RESET_GPHY0, 0);
    ar9_setup_ext_phy_reset(GPIO_RESET_GPHY1, 0);
...
int ar9_3port_sw_hw_init(void)
{
	...
	#define LANTIC_DMA_FIX
#if defined(LANTIC_DMA_FIX) 
	/*------------------------------------------------------------------------------------------*\
	 *  Fix von Lantiq um das Vollaufen des TX-DMA-Puffers zu vermeiden
	\*------------------------------------------------------------------------------------------*/

...
\endcode
   ...
   \section Dependency_Tables Dependency Tables of the subsystems
	...
	static IFX_PMCU_MODULE_DEP_t depList=
	{
		  10,
		  {
			 {IFX_PMCU_MODULE_VE,   1, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D2},
			 {IFX_PMCU_MODULE_PPE,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_USB,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_USB,  1, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_DSL,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_WLAN, 0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_DECT, 0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_GPTC, 0, IFX_PMCU_STATE_D0,   IFX_PMCU_STATE_D1, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_SPI,  0, IFX_PMCU_STATE_D0,   IFX_PMCU_STATE_D1, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D3},
			 {IFX_PMCU_MODULE_UART, 0, IFX_PMCU_STATE_D0,   IFX_PMCU_STATE_D1, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D3}
		  }
	};
	\endcode
...
/* PMCU driver version  */
#define IFX_PMCU_DRV_VERSION  "2.1.5.2"
...
/* max number of entries in dependency list 
   +1 represents the entry of the requested module inside the ifx_pmcu_preCall[][] array */
#define IFX_DEPLIST_SIZE (15 + 1)
...
#define PMCU_REQ(module,fmt,arg...)\
		if(log_level_g >= 2){\
		printk(KERN_ERR GREEN #module NORMAL ": " fmt,##arg);\
		}

#define PMCU_FUNC(module,fmt,arg...)\
		if(log_level_g >= 3){\
		printk(KERN_ERR GREEN #module NORMAL ": " fmt,##arg);\
		}
...
/* debug macro to print the content of the ifx_pmcu_ModMem */
	...
	for (i=0; i<IFX_PMCU_MODULE_ID_MAX;i++){ \
		for (j=0; j<MAX_PMCU_SUB_MODULE;j++){\
			PMCU_DEBUG(PMCU, "ifx_pmcu_ModMem[%d][%d]: pmcuModule=%s\t marker=%d\n",\
			i,j,ifx_pmcu_mod[i],ifx_pmcu_ModMem[i][j]); \
		} \
	} \
	}
#else
...
/*=============================================================================*/
   /* define dependency list;  state D0D3 means don't care */
   /* static declaration is necessary to let gcc accept this static initialisation. */
   /* we support currently 2 USB devices simultaneously; the DEVICE mode interface is */
   /* always registered on subnode_0; HOST mode interfaces sequentially, therfeore 3 USB entries. */
   /* Currently the USB module does not support a suitable D3 state, because in this */
   /* mode the interface does not detect an external device if it is plugged in. */
   /* Therefore all state are set with D0D3 which means don't care. */
  static IFX_PMCU_MODULE_DEP_t depList_CPU=
   {
      11,
      {
         {IFX_PMCU_MODULE_VE,   1, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D2},
         {IFX_PMCU_MODULE_PPE,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
         {IFX_PMCU_MODULE_USB,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3},
		 {IFX_PMCU_MODULE_USB,  1, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3},
		 {IFX_PMCU_MODULE_USB,  2, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3},
         {IFX_PMCU_MODULE_DSL,  0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
         {IFX_PMCU_MODULE_WLAN, 0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
         {IFX_PMCU_MODULE_DECT, 0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3, IFX_PMCU_STATE_D3},
         {IFX_PMCU_MODULE_GPTC, 0, IFX_PMCU_STATE_D0,   IFX_PMCU_STATE_D1, IFX_PMCU_STATE_D2, IFX_PMCU_STATE_D3},
   ...
   /* define dependency list;  state D0D3 means don't care */
   /* static declaration is necessary to let gcc accept this static initialisation. */
   static IFX_PMCU_MODULE_DEP_t depList_VE=
   {
      1,
      {
         {IFX_PMCU_MODULE_CPU, 0, IFX_PMCU_STATE_D0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3}
   ...
   /* define dependency list;  state D0D3 means don't care */
   /* static declaration is necessary to let gcc accept this static initialisation. */
   static IFX_PMCU_MODULE_DEP_t depList_OTH= 
   { 
      1,
      {
         {IFX_PMCU_MODULE_CPU, 0, IFX_PMCU_STATE_D0, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3, IFX_PMCU_STATE_D0D3}
...
*/
static IFX_uint8_t ifx_pmcu_ModMem[IFX_PMCU_MODULE_ID_MAX][MAX_PMCU_SUB_MODULE];
...
#ifdef CONFIG_PROC_FS
static IFX_int_t ifx_pmcu_proc_device_list(IFX_char_t *page, IFX_char_t **start, off_t offset, IFX_int_t count, IFX_int_t *eof, IFX_void_t *data);
...
static IFX_PMCU_RETURN_t ifx_pmcu_put_req( IFX_PMCU_REQ_STATE_t req )
{
	...
	for (i=0;i<IFX_REQ_BUFFER_SIZE;i++) {
		...
		if ((ifx_pmcu_reqBuffer[i].moduleState.pmcuModule == req.moduleState.pmcuModule) &&
			...
			PMCU_DEBUG(PMCU, "%s detect's multiple requests from same module. Discard request.\n",__FUNCTION__);
			return IFX_PMCU_RETURN_SUCCESS;
		}
	}
...
static IFX_void_t ifx_pmcu_process_req_tasklet(unsigned long data)
{
	...
	while (ifx_pmcu_reqBufferSize > 0) {
		...
		/* handle PMCU request */
		PMCU_INFO(PMCU, "REQ: pmcuModule=%s, pmcuModuleNr=%d, pmcuState=%s\n", ifx_pmcu_mod[reqState.moduleState.pmcuModule],
		...
		if (ret != IFX_PMCU_RETURN_SUCCESS) {
			...
			//PMCU_DEBUG(PMCU, "Error/Denied was returned from prechange or statechange\n");
		}
		...
		if (ret != IFX_PMCU_RETURN_SUCCESS) {
			...
			PMCU_ERR(PMCU, "Error/Denied was returned from ifx_pmcu_postchange()\n");
		}
...
IFX_PMCU_RETURN_t ifx_pmcu_state_req (IFX_PMCU_MODULE_t pmcuModule, IFX_uint8_t pmcuModuleNr, IFX_PMCU_STATE_t newState)
{
	...
	PMCU_REQ(PMCU, "%s is called: Module=%s, ModuleNr=%d, State=%s\n",__FUNCTION__,ifx_pmcu_mod[pmcuModule],pmcuModuleNr, ifx_pmcu_ps[newState]);
...
IFX_PMCU_RETURN_t ifx_pmcu_register (IFX_PMCU_REGISTER_t* pmcuRegister)
{
	...
	PMCU_FUNC(PMCU, "%s is called for module %s\n",__FUNCTION__,ifx_pmcu_mod[pmcuRegister->pmcuModule]);
	...
	if ( (entry->registryInfo[pmcuRegister->pmcuModuleNr] == IFX_PMCU_MODULE_NOT_REGISTRED) ||
		...
		if ( (pmcuRegister->pre != NULL) && (pmcuRegister->post != NULL) ) {
			...
			/* Start workaround for UGW5.2 only */
//			depSize = (sizeof(IFX_PMCU_STATES_DEP_t)) * (pmcuRegister->pmcuModuleDep->nDepth) + (sizeof(pmcuRegister->pmcuModuleDep->nDepth)); 
			switch (pmcuRegister->pmcuModule) { 
					case IFX_PMCU_MODULE_CPU:
						depSize =	(sizeof(IFX_PMCU_STATES_DEP_t)) * (depList_CPU.nDepth) + 
									(sizeof(depList_CPU.nDepth));
						pmcuRegister->pmcuModuleDep = &depList_CPU;
						break;
					case IFX_PMCU_MODULE_VE:
						depSize =	(sizeof(IFX_PMCU_STATES_DEP_t)) * (depList_VE.nDepth) + 
									(sizeof(depList_VE.nDepth)); 
						pmcuRegister->pmcuModuleDep = &depList_VE;
						break;
					default:
						depSize =	(sizeof(IFX_PMCU_STATES_DEP_t)) * (depList_OTH.nDepth) + 
									(sizeof(depList_OTH.nDepth));
						pmcuRegister->pmcuModuleDep = &depList_OTH;
						break;
				}
			/* End workaround for UGW5.2 only */
...
IFX_PMCU_RETURN_t ifx_pmcu_unregister (IFX_PMCU_REGISTER_t* pmcuUnregister)
{
	...
	down(&ifx_pmcu_map_mutex);
	spin_lock_irqsave(&ifx_pmcu_lock, iflags);
	PMCU_FUNC(PMCU, "%s is called for module %s\n",__FUNCTION__,ifx_pmcu_mod[pmcuUnregister->pmcuModule]);
...
static IFX_PMCU_RETURN_t ifx_pmcu_pwr_feature_switch (IFX_PMCU_MODULE_PWR_FEATURE_STATE_t pwrFeaSwitch)
{
	...
	PMCU_FUNC(PMCU, "%s is called for module %s\n",__FUNCTION__,ifx_pmcu_mod[pwrFeaSwitch.pmcuModule]);
	...
	if (ifx_pmcu_map[pwrFeaSwitch.pmcuModule].pmcuRegister[pwrFeaSwitch.pmcuModuleNr].ifx_pmcu_pwr_feature_switch == NULL) {
		PMCU_ERR(PMCU, "Module %s has no valid Callback defined for pwr_feature_switch\n",ifx_pmcu_mod[pwrFeaSwitch.pmcuModule]);
		return IFX_PMCU_RETURN_DENIED;
	}
	...
	if (ret != IFX_PMCU_RETURN_SUCCESS) {
		PMCU_ERR(PMCU, "Requested Power Feature State is not supported by module %s\n",ifx_pmcu_mod[pwrFeaSwitch.pmcuModule]);
		return IFX_PMCU_RETURN_DENIED;
	} else {
		if (pwrFeaSwitch.pmcuPwrFeatureState == IFX_PMCU_PWR_STATE_ON) {
			ifx_pmcu_map[pwrFeaSwitch.pmcuModule].dbgFlag &= ~(1 << pwrFeaSwitch.pmcuModuleNr);
		} else {
			ifx_pmcu_map[pwrFeaSwitch.pmcuModule].dbgFlag |= 1 << pwrFeaSwitch.pmcuModuleNr;
		}
	}  
	...
	if (ret != IFX_PMCU_RETURN_SUCCESS) {
		PMCU_ERR(PMCU, "Get state function return with error status for module %s\n",ifx_pmcu_mod[pmcuModState.pmcuModule]);
		return IFX_PMCU_RETURN_ERROR;
	} else {
...
static IFX_PMCU_RETURN_t ifx_pmcu_get_state (IFX_PMCU_MODULE_STATE_t* pmcuModState)
{
	...
	PMCU_FUNC(PMCU, "%s is called for module %s\n",__FUNCTION__,ifx_pmcu_mod[pmcuModState->pmcuModule]);
	...
	if (ifx_pmcu_map[pmcuModState->pmcuModule].pmcuRegister[pmcuModState->pmcuModuleNr].ifx_pmcu_state_get == NULL) {
		PMCU_WARNING(PMCU, "Module %s has no valid Callback defined for ifx_pmcu_state_get\n",ifx_pmcu_mod[pmcuModState->pmcuModule]);
		return IFX_PMCU_RETURN_NOTDEFINED;
	}
...
static IFX_PMCU_RETURN_t ifx_pmcu_parse_deplist (IFX_PMCU_MODULE_STATE_t pmcuModuleState)
{
	...
	PMCU_FUNC(PMCU, "%s is called for module=%s, moduleNr=%d,\n",__FUNCTION__,ifx_pmcu_mod[pmcuModuleState.pmcuModule],pmcuModuleState.pmcuModuleNr);
	...
	if (!ifx_pmcu_ModMem[pmcuModuleState.pmcuModule][pmcuModuleState.pmcuModuleNr]) {
		/* No. Module is not in the list. Create a new entry */
		ifx_pmcu_ModMem[pmcuModuleState.pmcuModule][pmcuModuleState.pmcuModuleNr] = 1; /* set marker */
	...
	for (i = 0; i < entryModDep->nDepth; i++) {
		switch (pmcuModuleState.pmcuState) { /* switch on requested powerState */
			case IFX_PMCU_STATE_D0:
				...
				pmcuModuleNr = entryModDep->moduleStates[i].pmcuModuleNr;
				break;
		...
		if (ifx_pmcu_ModMem[pmcuModule][pmcuModuleNr]) {
		...
		if (ifx_pmcu_stateMap[pmcuState][pmcuOldState] == 1) {
			...
			PMCU_DEBUG(PMCU, "no dependency activity necessary for module=%s, moduleNr=%d,\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
		}
	}
	return IFX_PMCU_RETURN_SUCCESS;
}
...
//   for (i = 0; i < ifx_pmcu_indexAccuDepL; i++) {
	for (i = (ifx_pmcu_indexAccuDepL-1); i >= 0; i--) {
		...
		if (ifx_pmcu_map[pmcuModule].registryInfo[pmcuModuleNr] != IFX_PMCU_MODULE_REGISTERED) {
			PMCU_ERR(PMCU, "module=%s, moduleNr=%d, listed in dependency list is not registered in PMCU\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			continue;
		}
		/* check if preCallback is defined */
		if (ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].pre == NULL) {
			PMCU_DEBUG(PMCU, "no preCallback defined for module=%s, moduleNr=%d\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			continue;
		}
		...
		if (ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].pre != NULL) {
			...
			PMCU_DEBUG(PMCU, "pre Callback of module=%s, moduleNr=%d, is called\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
		...
		if (ret == IFX_PMCU_RETURN_DENIED) {
			...
			PMCU_WARNING(PMCU, "preCallback of module=%s, moduleNr=%d, return DENIED\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			return IFX_PMCU_RETURN_DENIED;
		}
		if (ret == IFX_PMCU_RETURN_ERROR) {
			/* if one error will be returned, pre-processing can be interrupted */
			PMCU_ERR(PMCU, "preCallback of module=%s, moduleNr=%d, return ERROR\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			return IFX_PMCU_RETURN_DENIED;
		}
		if (ret == IFX_PMCU_RETURN_SUCCESS) {
			PMCU_DEBUG(PMCU, "module=%s, moduleNr=%d, accept preCallback\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
		} else {
			PMCU_DEBUG(PMCU, "module=%s, moduleNr=%d, NOACTIVITY in preCallback\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
		}
	}
	return IFX_PMCU_RETURN_SUCCESS;
}
...
static IFX_PMCU_RETURN_t ifx_pmcu_statechange (IFX_PMCU_MODULE_STATE_t pmcuModuleState, IFX_int8_t* pmcuPreCallIndex)
{
	...
	for (i = 0; i <= *pmcuPreCallIndex; i++) {
		...
		if (ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].ifx_pmcu_state_change != NULL) {
			PMCU_DEBUG(PMCU, "ifx_pmcu_state_change Callback of module=%s, moduleNr=%d, is called\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			ret = ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].ifx_pmcu_state_change(pmcuState);
			if ((ret == IFX_PMCU_RETURN_DENIED) || (ret == IFX_PMCU_RETURN_ERROR)) {
				PMCU_ERR(PMCU, "ifx_pmcu_state_change Callback of module=%s, moduleNr=%d, return DENIED or ERROR\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
				return IFX_PMCU_RETURN_ERROR;
			}
		}
	}
...
static IFX_PMCU_RETURN_t ifx_pmcu_postchange (IFX_PMCU_MODULE_STATE_t pmcuModuleState, IFX_int8_t* pmcuPreCallIndex)
{
	...
	for (i = 0; i <= *pmcuPreCallIndex; i++) {
		...
		if (ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].post != NULL) {
			PMCU_DEBUG(PMCU, "post Callback of module=%s, moduleNr=%d, is called\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			ret = ifx_pmcu_map[pmcuModule].pmcuRegister[pmcuModuleNr].post(pmcuModule, pmcuState, pmcuOldState);
		}
		if ((ret == IFX_PMCU_RETURN_DENIED) || (ret == IFX_PMCU_RETURN_ERROR)) {
			PMCU_ERR(PMCU, "post Callback of module=%s, moduleNr=%d, return DENIED or ERROR\n",ifx_pmcu_mod[pmcuModule],pmcuModuleNr);
			return IFX_PMCU_RETURN_ERROR;
		}
		if (ret == IFX_PMCU_RETURN_SUCCESS) {
...
static int ifx_pmcu_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case IFX_PMCU_STATE_GET:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_STATE_GET is called\n",__FUNCTION__);
		...
		case IFX_PMCU_STATE_REQ:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_STATE_REQ is called\n",__FUNCTION__);
				copy_from_user(&usrModuleState, (IFX_PMCU_MODULE_STATE_t *)arg, sizeof(IFX_PMCU_MODULE_STATE_t));
		...
		case IFX_PMCU_REQ_CTRL:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_REQ_CTRL is called\n",__FUNCTION__);
		...
		case IFX_PMCU_LOG_LEVEL:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_LOG_LEVEL is called\n",__FUNCTION__);
		...
		case IFX_PMCU_PWR_FEATURE_SWITCH:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_PWR_FEATURE_SWITCH is called\n",__FUNCTION__);
...
#ifdef CONFIG_PROC_FS
static int ifx_pmcu_proc_device_list(char *page, char **start, off_t offset, int count, int *eof, void *data)
{
	int len = 0;
	...
	IFX_PMCU_MODULE_STATE_t pmcuModState;
//	PMCU_INFO(PMCU_1,"buf=%p, offset=%lu, count=%d, datap=%p, eof=%p, len=%d\n",buf,(unsigned long)offset,count,data,eof,len);
	//PMCU_FUNC(PMCU, "%s is called\n",__FUNCTION__);
    if(offset > 0) {/*the read function is called from the kernel until it returns 0.*/
    	return 0;
    }
	len += sprintf(buf+len, "PMCU log_level = %d\n",log_level_g);
	...
	len += sprintf(buf+len, "\tf\t:power-saving-feature disabled (0=enabled, 1=disabled)(bit0=sub0,bit1=sub1,...,bit5=sub5)\n\n");
	len += sprintf(buf+len, "Modules\t\tSub0\tSub1\tSub2\tSub3\tSub4\tSub5\n");
	len += sprintf(buf+len, "\t\tr/ps/f\tr/ps/f\tr/ps/f\tr/ps/f\tr/ps/f\tr/ps/f\n");
	...
	for (i = 0; i < IFX_PMCU_MODULE_ID_MAX ; i++) {
		...
		for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {
			if ( (ifx_pmcu_map[i].registryInfo[j] != IFX_PMCU_MODULE_REGISTERED) || (ifx_pmcu_map[i].pmcuRegister[j].pmcuModule == IFX_PMCU_MODULE_PMCU) ) {
				...
				pmcuModState.pmcuModule = ifx_pmcu_map[i].pmcuRegister[j].pmcuModule;
				pmcuModState.pmcuModuleNr = j;
				if(IFX_PMCU_RETURN_SUCCESS == ifx_pmcu_get_state(&pmcuModState)){
					len += sprintf(buf+len,"%s/",ifx_pmcu_ps[pmcuModState.pmcuState]);
				}else{
					len += sprintf(buf+len,"%s/",ifx_pmcu_ps[ifx_pmcu_map[i].pmcuOldState[j]]);
				}
				flags = (ifx_pmcu_map[i].dbgFlag) & (1<<j);
				if (flags) {
					len += sprintf(buf+len,RED "%d\t" NORMAL,flags>>j);
				} else {
					len += sprintf(buf+len,"%d\t",flags>>j);
				}
			}
		}
		len += sprintf(buf+len,"\n");
...
//	PMCU_INFO(PMCU_2,"buf=%p, offset=%lu, count=%d, datap=%p, eof=%p, len=%d\n",buf,(unsigned long)offset,count,data,eof,len);
	return len;
}
...
//	PMCU_FUNC(PMCU_1,"buf=%p, offset=%lu, count=%d, datap=%p, eof=%p, len=%d\n",buf,(unsigned long)offset,count,data,eof,len);

	down(&ifx_pmcu_map_mutex);
	len += sprintf(buf + len, "pmcu_map[%d]->ModuleName=%s\n", (int)data, ifx_pmcu_map[(int)data].pmcuModuleName);
	for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {
		len += sprintf(buf + len, "regInfo [%d]=%d\t", j, ifx_pmcu_map[(int)data].registryInfo[j]);
	}
	len += sprintf(buf+len,"\n");
	for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {
		len += sprintf(buf + len, "OldState[%d]=%d\t", j, ifx_pmcu_map[(int)data].pmcuOldState[j]);
	}
	len += sprintf(buf+len,"\n");
	for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {
		len += sprintf(buf + len, "NewState[%d]=%d\t", j, ifx_pmcu_map[(int)data].pmcuNewState[j]);
	}
	len += sprintf(buf+len,"\n\n");
	for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {
		len += sprintf(buf + len, "Reg[%d]->Module=%d\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].pmcuModule);
		len += sprintf(buf + len, "Reg[%d]->ModuleNr=%d\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].pmcuModuleNr);
		len += sprintf(buf + len, "Reg[%d]->ifx_pmcu_state_change=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].ifx_pmcu_state_change);
		len += sprintf(buf + len, "Reg[%d]->ifx_pmcu_state_get=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].ifx_pmcu_state_get);
		len += sprintf(buf + len, "Reg[%d]->ifx_pmcu_pwr_feature_switch=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].ifx_pmcu_pwr_feature_switch);
		len += sprintf(buf + len, "Reg[%d]->pre=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].pre);
		len += sprintf(buf + len, "Reg[%d]->post=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].post);
		len += sprintf(buf + len, "Reg[%d]->ModuleDep=%p\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].pmcuModuleDep);
		if (ifx_pmcu_map[(int)data].pmcuRegister[j].pmcuModuleDep != NULL) {
			len += sprintf(buf + len, "Reg[%d]->ModuleDep->nDepth=%d\n", j, ifx_pmcu_map[(int)data].pmcuRegister[j].pmcuModuleDep->nDepth);
		}
		len += sprintf(buf+len,"\n");
	}
	for (j = 0; j < MAX_PMCU_SUB_MODULE; j++) {

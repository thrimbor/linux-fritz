...
static int proc_read_domain(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    for ( i = 0; i < NUM_ENTITY(g_rcu_domains); i++ ) {
        llen = print_reset_domain(proc_read_str, i);
        if ( len <= off && len + llen > off )
        {
            memcpy(pstr, proc_read_str + off - len, len + llen - off);
            pstr += len + llen - off;
        }
        else if ( len > off )
        {
            memcpy(pstr, proc_read_str, llen);
            pstr += llen;
        }
        len += llen;
        if ( len >= len_max )
        ...
        for ( j = 1, p_handler = g_rcu_domains[i].handlers; p_handler != NULL; j++, p_handler = p_handler->next ) {
            llen = print_reset_handler(proc_read_str, j, p_handler);
            if ( len <= off && len + llen > off )
            {
                memcpy(pstr, proc_read_str + off - len, len + llen - off);
                pstr += len + llen - off;
...
unsigned int ifx_rcu_rst_stat_read(void)
{
    ...
    spin_unlock_irqrestore(&g_rcu_register_lock, sys_flags);
    /*--- printk("IFX_RCU_RST_REQ %x\n", IFX_REG_R32(IFX_RCU_RST_REQ)); ---*/
    return ret;
}
...
pure_initcall(ifx_rcu_pre_init);   //  to be called earlier than other drivers as well as kernel modules

...
#define XHCI_FWFILENAME  "xhcifw.mem"
    ...
    for (offset = 0; offset < fw_dwordsize; offset++) {
        ...
        case SET_DATA_PAGE0:
          ...
          XHCI_DebugPrint("Confirm SET DATA0 is to be 0 is timeout(ERROR at writeto offset %04d)", offset);
            ...
            cfgpw (pDev, PCICNF0F8, cpu_to_le32(image_pointer[offset]));
            XHCI_DebugPrint("Set data 0 : offset = %04x, data = %08x", offset, cpu_to_le32(image_pointer[offset]));
            //d: Set "Set Data0".
            if(XHCI_SetControl(
                pDev,
                XHCI_CONTROLREGISTER_BIT_SET_DATA0
        ...
        case SET_DATA_PAGE1:
          ...
          XHCI_DebugPrint("Confirm SET DATA1 is to be 0 is timeout(ERROR at writeto offset %04d)", offset);
            ...
            cfgpw (pDev, PCICNF0FC, cpu_to_le32(image_pointer[offset]));
            XHCI_DebugPrint("Set data 1 : offset = %04x, data = %08x", offset, cpu_to_le32(image_pointer[offset]));
            //g: Set "Set Data1".
            if(XHCI_SetControl(
                pDev,
                XHCI_CONTROLREGISTER_BIT_SET_DATA1

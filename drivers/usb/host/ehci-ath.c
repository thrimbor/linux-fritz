...
static int ath_usb_ehci_init(struct usb_hcd *hcd)
{
	...
	/* == AVM/BC 20130923 usb_hub_available bedeutet usb_available */
	/* == AVM/HSCH 20120420 USB je nach HWConfig == */
	if (avm_get_hw_config(AVM_HW_CONFIG_VERSION, "usb_hub_available", 0, 0) < 0)
		return -ENODEV;
	...
	/* == AVM/BC 20110810 activate internal hub == */
	{
		int pin, ret;
		enum _avm_hw_param param;

		//check if hub reset pin is present
		ret = avm_get_hw_config(AVM_HW_CONFIG_VERSION, "gpio-usb_reset", &pin, &param);
		if(ret == 0) {

			WARN_ON(param != avm_hw_param_gpio_out_active_low);

			if(param == avm_hw_param_gpio_out_active_low) {
				avm_gpio_out_bit(pin, 1);
			}
			  ...
			  struct platform_device *pdev)
{
	...
	if(pdev->id == ATH_USB_HOST0) {
		...
		/* AVM/BC 25.01.2013: Soft Reset Sequence from WASP */
		...
		dev_dbg(&pdev->dev, "error in host selection\n");
	}
#else
...
#if defined(CONFIG_USB_EHCI_ATH_HOST1) && defined(CONFIG_USB_EHCI_ATH_HOST2)
...
#if 0   /*--- #ifdef CONFIG_MACH_QCA955x ---*/

 ...
 */
#include <asm/mach-infineon/ifx_types.h>
#include <asm/mach-infineon/ifx_regs.h>
#include <asm/mach-infineon/common_routines.h>
#include <asm/mach-infineon/ifx_led.h>
#include <asm/mach-infineon/ifx_clk.h>
...
// TKL: CPE_MEI nicht im Kernel einkonfiguriert, wird aber vom DSL_SDK implementiert
#if defined(CONFIG_IFXMIPS_DSL_CPE_MEI) || defined(CONFIG_IFXMIPS_DSL_CPE_MEI_MODULE) || defined(CONFIG_IFX_PPA_AVM_USAGE)
...
static char *g_net_dev_name[1] = {"ptm_vr9"};
...
static void ptm_setup(struct net_device *dev, int ndev)
{
  ...
  #ifdef CONFIG_IFX_PTM_RX_NAPI
    netif_napi_add(dev, &g_ptm_priv_data.itf[ndev].napi, ptm_napi_poll, 64);
  #endif
#endif
...
#ifdef CONFIG_AVM_PA
            if (avm_pa_dev_receive(AVM_PA_DEVINFO(g_net_dev[0]), skb) != AVM_PA_RX_ACCELERATED) {
#endif
...
static int ptm_napi_poll(struct net_device *dev, int *quota)
{
    ...
    *quota -= work_done;
	...
	/* FIX crash in net_rx_action: 
       AVM GRM: do not call netif_rx_complete if work_done == budget */
...
static int proc_read_prio(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + off + len,    "  ptm_vr9     :");
...
static int proc_write_prio(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        {
            printk("echo <ptm_vr9> prio xx queue xx [prio xx queue xx] > /proc/driver/ifx_ptm/prio\n");
            break;
        }
        ...
        else if ( stricmp(p1, "ptm_vr9") == 0 )
        {
            port = 2;
            prio = queue = -1;
            printk("port = 2 (PTM_VR9)\n");
        }
...
static int proc_write_dbg(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    else if ( strincmp(p, "leave", 5) == 0 && !IS_ERR(&ifx_mei_atm_showtime_exit) && &ifx_mei_atm_showtime_exit && ifx_mei_atm_showtime_exit != NULL )
...
{
    skb->dev = (struct net_device *) arg; /* wird nicht vom AVM PA gesetzt */
    dev_queue_xmit(skb);
}
#endif
...
#ifdef CONFIG_AVM_PA
        {
			struct net_device *dev = g_net_dev[i];
            ...
            if(avm_pa_dev_pid_register(AVM_PA_DEVINFO(dev), &cfg) < 0)
                ...
                printk(KERN_INFO "%s: registered to AVM PA\n", cfg.name);
        }
#endif
    }
...
static int __init wanqos_en_setup(char *line) __attribute__((unused));
...
static int __init queue_gamma_map_setup(char *line) __attribute__((unused));

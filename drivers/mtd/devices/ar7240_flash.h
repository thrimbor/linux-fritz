...
#define ar7240_spi_go() do {        \
    ath_reg_wr_nf(ATH_SPI_WRITE, AR7240_SPI_CE_LOW); \
...
#define ar7240_spi_done()       { do ath_reg_wr(ATH_SPI_FS, 0); while(ath_reg_rd(ATH_SPI_FS) == 1); } /*--- mbahr: workaround: seems sometimes unreliable ---*/

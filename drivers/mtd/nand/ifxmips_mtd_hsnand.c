...
static void ifx_hsnand_write_buf(struct mtd_info *mtd, const u8 *buf, int len)
{
    ...
    //printk("tx_ndac1: %08x, tx_ndac2: %08x\n", IFX_REG_R32(IFX_NDAC_CTL1), IFX_REG_R32(IFX_NDAC_CTL2));
...
static void update_hsnand_addr_lp(struct nand_chip *chip, int page_addr, int column, int cmd)
{
    ...
    addr_1 = addr_1 << 8;
    addr_2 = page_addr & 0x000000FF;
    addr_2 = addr_2 << 24;

    hsnand->ndac_ctl_1 = addr_2 | addr_1 | addr_0 | cmd;

    addr_3 = page_addr & 0x0000FF00;
    addr_3 = addr_3 >> 8;
    addr_4 = page_addr & 0x00FF0000;
    addr_4 = addr_4 >> 8;

    hsnand->ndac_ctl_2 = 0x40230000 | addr_4 | addr_3;
                         ...
                         int column, int page_addr)
{
        ...
        if ((command == NAND_CMD_SEQIN) && (column < mtd->writesize)) 
        {
            if (mtd->writesize <= 512)
                ...
                update_hsnand_addr_lp(chip, page_addr, column, HSNAND_WRITE);
        ...
        if (command == NAND_CMD_READ0 && (column < mtd->writesize))
        {
            if (mtd->writesize <= 512)
                ...
                update_hsnand_addr_lp(chip, page_addr, column, HSNAND_READ);
                            ...
                            int column, int page_addr)
{
	...
	asm("sync");
		...
		//ndelay(100);
        	...
        	DEBUG(MTD_DEBUG_LEVEL1, "Reading @ column 0x%x, page: %d\n", 
                      column, page_addr);
...
/* initialize DMA */
static int hsnand_dma_setup(void)
{
	...
	dma_device->rx_chan[i]->packet_size = 512;
...
static int __init ifx_hsnand_init(void)
{
   ...
   // IFX_REG_W32(IFX_REG_R32(IFX_DMA_CTRL) | (1<<8), IFX_DMA_CTRL);
    printk("DMA Readback: %08x\n", IFX_REG_R32(IFX_DMA_CTRL));

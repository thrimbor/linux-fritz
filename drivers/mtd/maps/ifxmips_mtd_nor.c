...
ifx_mtd_map_copy_from(struct map_info *map, void *to, unsigned long from, ssize_t len)
{
    ...
    IFX_MTD_PRINT("from:%lx to:%p len:0x%x\n", from, to, len);
...
ifx_mtd_map_write(struct map_info *map, const map_word datum, unsigned long ofs)
{
    if (map_bankwidth_is_1(map)) {
        ...
        *(u16 *)(map->map_priv_1 + ofs) = (u16)datum.x[0];
        ...
        IFX_MTD_PRINT("map_bankwidth_is_2 w16: [0x%08x + 0x%lx] <== %x\n",
...
ifx_mtd_map_copy_to(struct map_info *map, unsigned long to, const void *from,
        ssize_t len)
{
    IFX_MTD_PRINT("from:0x%x to:0x%x len:0x%x", (unsigned int)from, (unsigned int)to, len);
...
/* XXX, VR9 support */     
    ...
    if (!start_scan_addr) {
        IFX_MTD_PRINT("Failed to ioremap address:0x%lx\n", flash_start);
        return -EIO;
    }
    
    IFX_MTD_PRINT("start_scan_addr: 0x%lx, flash_start: 0x%lx flash_size 0x%lx\n",
    ...
    for (idx = 0; idx < IFX_NOR_FLASH_BANK_MAX; idx++) {
        ...
        memset( map_bank->name, 0, IFX_MTD_NOR_NAME_LEN);

        /* Bank name must be the same as command line bank name */
        sprintf(map_bank->name, "%s%d", IFX_MTD_NOR_BANK_NAME, idx);

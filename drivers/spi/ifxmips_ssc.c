...
#define IFX_SSC_VER_MID            4
#define IFX_SSC_VER_MINOR          1
...
static void ifx_ssc_dump_register(void);
...
/**
 * \fn static INLINE void ifx_ssc_raise_fake_irq(struct ifx_ssc_port *port, unsigned int irq)
 ...
 */
static INLINE void 
ifx_ssc_raise_fake_irq(struct ifx_ssc_port *port, unsigned int irq)
{
    unsigned long flags;
...
ifx_ssc_start_tasklet(struct ifx_ssc_port *port)
{
    ...
    if (in_irq()) { /* Hardware irq */
        ...
        ifx_ssc_raise_fake_irq(port, port->ssc_fake_irq);
    }
}
...
{
    unsigned int cntdown = 1000 * 1000;
    while((IFX_SSC_GET_STATE(port) & IFX_SSC_STATE_BUSY)){
        udelay(1)
        ; /* Do nothing */
        if(cntdown-- == 0) {
            /*--- ifx_ssc_dump_register(); ---*/
            printk(KERN_ERR"%s: in BUSY_STATE\n", __func__);
            break;
        }
    }
}
    ...
    for (t = 0; t < IFX_SSC_MAX_PORT_NUM; t++) {
        ...
        TAILQ_FOREACH(dev, &port->ssc_devq, dev_entry){
            off += sprintf(page + off, "Device %s duplex %s:\n", dev->dev_name, 
                (dev->duplex == IFX_SSC_HALF_DUPLEX) ? "Half" : "Full");
            off += sprintf(page + off, "Rx Bytes %llu\n", dev->stats.rxBytes);
            off += sprintf(page + off, "Tx Bytes %llu\n", dev->stats.txBytes);
...
static void ifx_ssc_dump_register(void) {
    char tmp[768];
    int eof;
    ifx_ssc_reg_read_proc(tmp, NULL, 0, 0, &eof, NULL);
...
#endif/*--- #ifdef IFX_SSC_DMA_SUPPORT ---*/
...
ifx_ssc_dma_int_handler(struct dma_device_info* dma_dev, int status)
{
    ...
    switch(status) {
        case RCV_INT:
            ...
            /* 
             * NB, we have to reset SPI DMA Rx channel every time, just it is not enough to
...
/*--- #define SSC_TIMING_CHECK ---*/
#if defined(SSC_TIMING_CHECK)
#define CLK_TO_USEC(a) ((a) / 250)
#define CLK_TO_NSEC(a) ((a * 1000) / 250)
struct _generic_stat {
    signed long cnt; 
    signed long avg;
    signed long min;
    signed long max;
} ssc_timestat_rx, ssc_timestat_tx, ssc_timestat_rxtx, ssc_cntstat_rx, ssc_cntstat_tx;
static DEFINE_SPINLOCK(ssc_stat_lock);
/*--------------------------------------------------------------------------------*\
\*--------------------------------------------------------------------------------*/
...
void generic_stat(struct _generic_stat *pgstat, signed long val) {
    unsigned long flags;
    spin_lock_irqsave(&ssc_stat_lock, flags);
    if(pgstat->cnt == 0) {
    ...
    pgstat->avg += val;
    pgstat->cnt++;
    spin_unlock_irqrestore(&ssc_stat_lock, flags);
}
/*--------------------------------------------------------------------------------*\
 * timebase: 0 unveraendert, 1: clk ->  usec, 2: clk -> nsec
 * reset: Statistik ruecksetzen
\*--------------------------------------------------------------------------------*/
void display_generic_stat(const char *prefix, struct _generic_stat *pgstat, unsigned int timebase, unsigned int reset) {
    unsigned long flags;
...
#endif/*--- #else ---*//*--- #if defined(SSC_TIMING_CHECK) ---*/

static void ifxssc_reset_rx_fifo(struct ifx_ssc_port *port) {
    unsigned int reg;
    IFX_SSC_IRQ_LOCK(port);
    printk("%s: rx-error occured, flush rx-fifo\n", __func__);
    reg = IFX_SSC_XFCON_FIFO_FLUSH  | IFX_SSC_XFCON_FIFO_ENABLE | SM((port->rx_fifo_size_words >> 1), IFX_SSC_XFCON_ITL);
    IFX_SSC_RX_FIFO_CTRL(reg, port);
    IFX_SSC_SET_WHBSTATE(IFX_SSC_WHBSTATE_CLR_RX_OFL_ERROR , port);
    IFX_SSC_IRQ_UNLOCK(port);
}
...
#ifdef DBG_SET_BAUDRATE
    printk(KERN_ERR "[%x:%s/%d] baudrate=%u, ifx_ssc_clock=%u CPHY1=%x\n", smp_processor_id(), __FUNCTION__, __LINE__,
            baudrate, ifx_ssc_clock, IFX_REG_R32(IFX_GPHY1_CFG));
#endif /*--- #ifdef DBG_SET_BAUDRATE ---*/
#ifdef CONFIG_VR9
...
#ifdef DBG_SET_BAUDRATE
    printk(KERN_ERR "[%d:%s/%d] br(divider)=%u\n", smp_processor_id(), __FUNCTION__, __LINE__,
            br);
#endif /*--- #ifdef DBG_SET_BAUDRATE ---*/
    smp_wmb();
    if (br > 0xffff ||((br == 0) &&
...
{   
    ...
    if (baudrate > IFX_SSC_SPLIT_BAUD_RATE) {
        ...
        reg |= SM(IFX_SSC_CON_CLK_DELAY_DEFAULT, IFX_SSC_CON_CLK_DELAY);
        IFX_SSC_SET_CON(reg, port);
    }
    else {
        reg = IFX_SSC_GET_CON(port);
...
ifx_ssc_hwinit(struct ifx_ssc_port *port)
{
    ...
    IFX_SSC_IRQ_LOCK(port);
    /*
     * With this setting it is assured, that if the DMA stalls (RX cannot write 
     * data and TX cannot read data) at a certain point in time, only one 
     * additional TX burst (2 words) + one word from the burst that is just 
     * processing at maximum will be forwarded by the SSC TX FIFO towards 
     * the line side. In this case I assume that at maximum 4 words should be 
     * available within the RX FIFO (2 words from ongoing TX burst + 2 additional 
     * words from the last TX burst requested before the DMA stall (requested as 
     * the TX FIFO level reached 1). In worst case timing may be 6 words remain 
     * within the RX FIFO (I'm not 1000% sure here), but at least we should not 
     * face any overflow.
     */
    
    /* TX FIFO filling level, half of FIFO size */
    reg = SM(1/*(port->tx_fifo_size_words >> 1)*/, IFX_SSC_XFCON_ITL) |
    ...
    reg = SM( 2 /*(port->rx_fifo_size_words - 2)*/, IFX_SSC_XFCON_ITL);
#else
...
/** 
 * \fn static int ifx_ssc_start_rx_last_word(struct ifx_ssc_port *port, char *buf, size_t size)
...
/** 
 * \fn static int ifx_ssc_start_tx_last_word(struct ifx_ssc_port *port, char *buf, size_t size)
 * \brief Called to transmit last word <1 ~ 4> bytes for full duplex operation.  
 *
 * \param   port       Pointer to structure #ifx_ssc_port
 ...
 * \ingroup IFX_SSC_INTERNAL 
 */
#define IFX_SSC_WAIT(_p)        IFX_SSC_GET_STATE(_p) & IFX_SSC_STATE_BUSY
...
ifx_ssc_start_tx_last_word(struct ifx_ssc_port *port, char *buf, size_t size)
{
    switch (size) {
        ...
        case 3:
            IFX_SSC_TX_HALFWORD(*(u16 *)buf, port);
            while (IFX_SSC_WAIT(port));             /*--- ohne WAIT geht das 3. Byte verloren ---*/
...
ifx_ssc_start_txfifo(struct ifx_ssc_port *port, char *txbuf, size_t txsize)
{
    ...
    int total = txsize;
#if defined(SSC_TIMING_CHECK)
    unsigned long ssc_start_time;
#endif/*--- #if defined(SSC_TIMING_CHECK) ---*/
    ...
    for (;;) {
        ...
        if (total <= 4) {
            ...
            ifx_ssc_start_tx_last_word(port, (char *)ptxbuf, tx_cnt);
        }
        txbuf += eff_bytes;
        total -= eff_bytes;
        if (total == 0){
    ...
    {
        static int cnt;
        generic_stat(&ssc_timestat_tx, get_cycles() - ssc_start_time);
        generic_stat(&ssc_cntstat_tx, txsize);
        if(cnt++ > 1000) {
            cnt = 0;
            display_generic_stat(__func__, &ssc_timestat_tx, 1, 1);
            display_generic_stat("txsize", &ssc_cntstat_tx, 0, 1);
        }
    }
#endif/*--- #if defined(SSC_TIMING_CHECK) ---*/
    IFX_SSC_IRQ_UNLOCK(port);
...
ifx_ssc_start_rxfifo(struct ifx_ssc_port *port, char *rxbuf, size_t rxsize)
{
    ...
    for (;;) {
        ...
        if (total >= port->rx_fifo_size_bytes) {
            ...
            ifx_ssc_start_rx_last_word(port, (char *)prxbuf, rx_cnt);
        }
        total -= eff_bytes;
        rxbuf += eff_bytes;
        if (total == 0){
    ...
    if((IFX_SSC_GET_STATE(port) & IFX_SSC_STATE_RX_OFL)){
        printk(KERN_ERR "[%x]%s RX OverFlow, please reduce baudrate(%d)\n", smp_processor_id(), __func__, port->baudrate);
        ifxssc_reset_rx_fifo(port);
        ifx_ssc_dump_register();
        rxsize = -EIO; 
    }
#if defined(SSC_TIMING_CHECK)
    {
        static int cnt;
        generic_stat(&ssc_timestat_rx, get_cycles() - ssc_start_time);
        generic_stat(&ssc_cntstat_rx, rxsize);
        if(cnt++ > 1000) {
            cnt = 0;
            display_generic_stat(__func__, &ssc_timestat_rx, 1, 1);
            display_generic_stat("rxsize", &ssc_cntstat_rx, 0, 1);
        }
    }
#endif/*--- #if defined(SSC_TIMING_CHECK) ---*/
    IFX_SSC_IRQ_UNLOCK(port);
...
/** 
 * \fn static int ifx_ssc_txrx_fifo_fdx(struct ifx_ssc_port *port, char *txbuf, u32 txsize, char *rxbuf, u32 rxsize)
 ...
 */
static int
ifx_ssc_txrx_fifo_fdx(struct ifx_ssc_port *port, char *txbuf, u32 txsize, char *rxbuf, u32 rxsize)
{
    int i;
    int tx_fifo_space;
    int rx_fill_space;
    ...
    while (1) {
        ...
        tx_fifo_space = port->tx_fifo_size_words - IFX_SSC_TX_FIFO_FILL_LEVEL(port);
        /* No space, wait */
        if (tx_fifo_space == 0) {
            continue;
        }
    ...
    {
        static int cnt;
        generic_stat(&ssc_timestat_rxtx, get_cycles() - ssc_start_time);
        generic_stat(&ssc_cntstat_tx, txsize);
        generic_stat(&ssc_cntstat_rx, rxsize);
        if(cnt++ > 1000) {
            cnt = 0;
            display_generic_stat(__func__, &ssc_timestat_rxtx, 1, 1);
            display_generic_stat("txsize", &ssc_cntstat_tx, 0, 1);
            display_generic_stat("rxsize", &ssc_cntstat_rx, 0, 1);
        }
    }
...
{
    IFX_KASSERT(port->opts.modeRxTx == IFX_SSC_MODE_TX, ("%s invalid txrx mode\n", __func__));
...
{
    IFX_KASSERT(port->opts.modeRxTx == IFX_SSC_MODE_RX, ("%s invalid txrx mode\n", __func__));
...
{
    static unsigned int old_baud = 0;
    IFX_SSC_CONFIGURE_t *ssc_cfg;
    struct ifx_ssc_port *port;
    ...
    if(old_baud == dev->conn_id.baudrate) {
        /*--- printk(KERN_ERR "[ifx_ssc_cs_lock] 1\n"); ---*/
        if (dev->conn_id.csset_cb != NULL) {
            dev->conn_id.csset_cb(IFX_SSC_CS_ON, dev->conn_id.cs_data);
        }
        port->ssc_cs_locked = IFX_TRUE;
        return;
    }
    /*--- printk(KERN_ERR "[ifx_ssc_cs_lock] 2\n"); ---*/
    IFX_SSC_PRINT(port, SSC_MSG_LOCK, "%s enter\n", __func__);
...
ifx_sscLock(IFX_SSC_HANDLE handler)
{
    ...
    if(atomic_add_return(1, &pqueue->isqueued) > 1) {
        printk(KERN_ERR"%s: error: isqueued\n", __func__);
        return (-1);
    }
...
/**
 * \fn static int ifx_ssc_dma_fdx(struct ifx_ssc_port *port, char *txbuf, int txsize, char *rxbuf, int rxsize)
 ...
 */
static int
ifx_ssc_dma_fdx(struct ifx_ssc_port *port, char *txbuf, int txsize, char *rxbuf, int rxsize)
{
    int retval = 0;
    struct dma_device_info* dma_dev;
    dma_dev = port->dma_dev;
...
ifx_sscTxRx(IFX_SSC_HANDLE handler, char *txbuf, u32 txsize, char *rxbuf, u32 rxsize)
{
    ...
    if (in_interrupt()) {
        stats->context_err++;
        if(stats->context_err < 5) {
            printk(KERN_ERR "%s can't be called in interupt context< irq, softirq, tasklet>%s\n", __func__,
                   stats->context_err == 4 ? "... supressing further error reports\n" : "" );
            dump_stack();
        }
        return 0;
    }
    ...
    */
    ...
    if(atomic_add_return(1, &pqueue->isqueued) > 1) {
        stats->dup_qentries++;
        IFX_SSC_PRINT(port, SSC_MSG_ERROR, "%s same queue has queued more than once(%s)\n", __func__, current->comm);
        return 0;
    }
...
ifx_ssc_serve_qentry(struct ifx_ssc_port *port)
{
    ...
    dma_dev = port->dma_dev;
#endif /* IFX_SSC_DMA_SUPPORT */
    if ((ssc_cfg->duplex_mode == IFX_SSC_FULL_DUPLEX) && (qentry->txbuf != NULL) 
        && (qentry->rxbuf != NULL)) {
    #ifdef IFX_SSC_DMA_SUPPORT
        ...
        if ((qentry->txsize > ssc_cfg->maxFIFOSize) && (tx_dma_aligned == 1) &&
            (qentry->rxsize > ssc_cfg->maxFIFOSize) && (rx_dma_aligned == 1)) {

            IFX_SSC_PRINT(port, SSC_MSG_DUPLEX, "%s TX/RX DMA enter\n", __func__);
            ifx_ssc_dma_fdx(port, qentry->txbuf, qentry->txsize, qentry->rxbuf, qentry->rxsize);
    ...
    #endif /* IFX_SSC_DMA_SUPPORT */
        {
            ifx_ssc_txrx_fifo_fdx(port, qentry->txbuf, qentry->txsize, qentry->rxbuf, qentry->rxsize);
            port->stats.txFifo++;
            port->stats.rxFifo++;
            IFX_SSC_PRINT(port, SSC_MSG_DUPLEX, "%s TX/RX FIFO enter\n", __func__);
...
ifx_sscAllocConnection(char *dev_name, IFX_SSC_CONFIGURE_t *connid)
{
    ...
    if (connid->fragSize < IFX_SSC_MIN_FRAGSIZE 
        ...
        IFX_SSC_PRINT(port, SSC_MSG_ERROR, "%s %d invalid fragment size <%d~%d>!\n", __func__,
            connid->fragSize,IFX_SSC_MIN_FRAGSIZE, IFX_SSC_MAX_FRAGSIZE);
        return NULL;
    }

    if (connid->maxFIFOSize < IFX_SSC_FIFO_MIN_THRESHOULD 
        || connid->maxFIFOSize > IFX_SSC_FIFO_MAX_THRESHOULD) {
        IFX_SSC_PRINT(port, SSC_MSG_ERROR, "%s %d invalid fifo size <%d~%d>!\n", __func__,
            connid->maxFIFOSize, IFX_SSC_FIFO_MIN_THRESHOULD, IFX_SSC_FIFO_MAX_THRESHOULD);
        return NULL;
    }

    if (connid->duplex_mode != IFX_SSC_FULL_DUPLEX && connid->duplex_mode != IFX_SSC_HALF_DUPLEX) {
        IFX_SSC_PRINT(port, SSC_MSG_ERROR, "%s %d invalid duplex mode <%d~%d>!\n", __func__,
            connid->duplex_mode, IFX_SSC_FULL_DUPLEX, IFX_SSC_HALF_DUPLEX);
        return NULL;
    }

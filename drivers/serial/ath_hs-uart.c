...
/*--- #define CONFIG_ATH_HS_UART_CONSOLE ---*/

unsigned int ath_hs_baud = ATH_HS_UART_BAUD;
...
void ath_hs_uart_baud_init(unsigned int baud)
{
	...
	clk_scale = ((serial_clk >> 17) * 1310) / baud;

	//clk_step = ((128k * 115200 * (clk_scale + 1)) / serial_clk)
	// Splitting 128K as 128 * 1024
	clk_step = ((128 * (baud / 100) * (clk_scale + 1)) << 10)
			/ (serial_clk / 100);
	...
	data = (data & ~GPIO_OUT_FUNCTION2_ENABLE_GPIO_10_MASK) |
		GPIO_OUT_FUNCTION2_ENABLE_GPIO_10_SET(ATH_HS_UART1_TD);
	...
	data = (data & ~GPIO_OUT_FUNCTION3_ENABLE_GPIO_12_MASK) |
		GPIO_OUT_FUNCTION3_ENABLE_GPIO_12_SET(ATH_HS_UART1_RTS);
	...
	ath_sys_frequency();
	printk("[%s] step 0x%x scale 0x%x\n", __FUNCTION__, clk_step, clk_scale);
...
{
    printk(KERN_ERR "[%s] open as device %d\n", __FUNCTION__, tty->index);
	if (tty->index >= tty->driver->num) {
        printk(KERN_ERR "[%s] open as device %d >= driver num %d\n", __FUNCTION__, tty->index, tty->driver->num);
		return -ENODEV;
    }
	return 0;
}
...
{
    printk(KERN_ERR "[%s] close device\n", __FUNCTION__, tty->index);
	return;
}
			...
			const unsigned char *buf, int count)
{
    ...
    printk(KERN_ERR "[%s] write %d bytes '%s'\n", __FUNCTION__, count, buf);
...
static irqreturn_t ath_hs_uart_isr(int irq, void *dev_id)
{
    ...
    /*--- code aus UartPut arch/mips/atheros/setup.c ---*/
	do {
...
static int __init ath_hs_uart_user_init(void)
{
	...
	tty->init_termios.c_ispeed = tty->init_termios.c_ospeed = ath_hs_baud;

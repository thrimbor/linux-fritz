...
static int vpe_run(struct vpe * v)
{
	...
	printk(KERN_WARNING "%s: tc %d start-addr %lx\n", __func__, t->index, v->__start);
    ...
    printk(KERN_WARNING "%s: tc %d tcstatus %08lx tcbind %08lx\n MVPCONTROL %08x vpeopt %08x\n",
                                                                                         __func__,
                                                                                         t->index,
                                                                                         read_tc_c0_tcstatus(), 
                                                                                         read_tc_c0_tcbind(),
                                                                                         read_c0_mvpcontrol(),
                                                                                         __read_32bit_c0_register($1,7));
...
static int vpe_elfload(struct vpe * v)
{
	...
	if (relocate) {
	    pr_info("VPE loader: loading to base %p (relocateable)\n", v->load_addr);
		...
		for (i = 0; i < hdr->e_phnum; i++) {
			if (phdr->p_type == PT_LOAD) {
	            pr_info("VPE loader: loading segment %d to %p (%x bytes)\n", i, (void *)phdr->p_paddr, phdr->p_memsz);
                ...
                if(phdr->p_memsz) {    
                    dma_cache_wback_inv((unsigned long)phdr->p_paddr, phdr->p_memsz);
                    flush_icache_range((unsigned long)phdr->p_paddr, (unsigned long)phdr->p_paddr + phdr->p_memsz);
                }
		    }
		    phdr++;
		}
...
static void __init smvp_tc_init(unsigned int tc, unsigned int mvpconf0) {
	...
	if (!tc) {
        printk("%s: tc %d tcbind %08lx tcstatus %08lx\n", __func__, tc, read_tc_c0_tcbind(), read_tc_c0_tcstatus());
		return;
    }
...
/*--------------------------------------------------------------------------------*\
\*--------------------------------------------------------------------------------*/
static unsigned int __init smvp_vpe_init(unsigned int tc, unsigned int mvpconf0, unsigned int ncpu) {
...
static int __init vpe_module_init(void)
{
	...
	int tc, err = 0;
#if 0
	printk(KERN_ERR "vorher in_atomic(): %d, irqs_disabled(): %d, pid: %d, name: %s \n",
    			in_atomic(), irqs_disabled(), get_current()->pid, get_current()->comm);
	
    local_irq_save(flags);

	printk("[%s] irq_flags %#x \n", __FUNCTION__, (unsigned int)flags); 
	printk(KERN_ERR "drin in_atomic(): %d, irqs_disabled(): %d, pid: %d, name: %s \n",
    			in_atomic(), irqs_disabled(), get_current()->pid, get_current()->comm);

	local_irq_restore(flags);
	printk(KERN_ERR "nachher in_atomic(): %d, irqs_disabled(): %d, pid: %d, name: %s \n",
    			in_atomic(), irqs_disabled(), get_current()->pid, get_current()->comm);
...
|| (tclimit >= hw_tcs)
	...
	for (tc = tclimit; tc < hw_tcs; tc++) {
		...
		if (tc < hw_tcs) {
...
#if 0
            printk( KERN_ERR "[%s] alloc_vpe(tc), tc=%d\n", __FUNCTION__, tc);

	        printk( KERN_ERR "in_atomic(): %d, irqs_disabled(): %d, pid: %d, name: %s \n",
                    in_atomic(), irqs_disabled(), get_current()->pid, get_current()->comm);
#endif
           /*------------------------------------------------------------*\
            * Irqs wieder zulassen waehrend alloc_vpe
           \*------------------------------------------------------------*/
           ...
           /*------------------------------------------------------------*\
            * Irqs wieder verbieten nach alloc_vpe
           \*------------------------------------------------------------*/
			...
			if (tc >= tclimit) {
				unsigned long tmp = read_vpe_c0_vpeconf0();
                /*--------------------------------------------------------------------------------*\
                Virtual Processor Activated: If zero, no TCs bound to this VPE will
                run. It is reset to 1 for VPE0, 0 for others.
                \*--------------------------------------------------------------------------------*/
				tmp &= ~VPECONF0_VPA;
                /*--------------------------------------------------------------------------------*\
                Master Virtual Processor: If set, registers in different VPEs (or in the
                TCs of different VPE afﬁliation) are made writable. It also controls
                read/write access to MVPControl (see Section 7.2.2 “MVPControl
                Register (CP0 Register 0, Select 1)”). It is reset to 1 for VPE0, 0 for
                others.
                \*--------------------------------------------------------------------------------*/
				tmp |= VPECONF0_MVP;
				write_vpe_c0_vpeconf0(tmp); /*--- mbahr@avm: fix: auskommentiert sonst bleibt kernel haengen ---*/
			}
   ...
   /*--------------------------------------------------------------------------------*\
    Set to write to conﬁguration register ﬁelds which are otherwise read-
    only on conventional MIPS32® CPUs. Unset to make the ﬁelds in
    the ConﬁgNN registers read-only.
    \*--------------------------------------------------------------------------------*/
    set_c0_mvpcontrol(MVPCONTROL_VPC);
	for (tc = 0; tc < hw_tcs; tc++) {
		settc(tc);
		...
		smvp_vpe_init(tc, mvpconf0, hw_vpes);
	}
    }
#endif
    dump_mtregs();

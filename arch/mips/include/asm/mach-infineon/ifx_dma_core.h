...
/** Max num of dma channels */
#define MAX_DMA_CHANNEL_NUM             20  
/*--- #define CONFIG_NAPI_ENABLED ---*/ /*--- XXX for testing ---*/
...
/** Not start of Packet */
#define IFX_DMA_NOT_SOP             (1 << 31)

/** Not end of Packet */
#define IFX_DMA_NOT_EOP             (1 << 30)

/** IFX_DMA_CHANNEL_LOCK was acquired by upper layer **/
#define IFX_DMA_LOCK_BY_UPPER_LAYER (1 << 29)
...
typedef struct dma_channel_info {
    ...
    volatile int curr_desc;          /*!< Current Descriptor number*/
    volatile int prev_desc;          /*!< Previous Descriptor number */
    ...
    int channel_complete_irq_counter;
    int channel_full_irq_counter;
    unsigned long channel_hangs_since;
    int peri_to_peri;       /*!< Config Peripheral to Peripheral(not supported Danube) */
    int no_cpu_data;	    /* added by avm: this channel is a direct path channel (data is not going through mips-cpu) */
...
typedef struct dma_device_info {
    struct list_head parachute_list;
    ...
    unsigned int	num_tx_chan;            /*!< Config the num of Tx channels for device */
    unsigned int	num_rx_chan;            /*!< Config the num of Rx channels for device */
    unsigned int max_rx_chan_num;        /*!< Max number of Rx channels supported */
    unsigned int max_tx_chan_num;        /*!< Max number of Tx channels supported */
    spinlock_t  irq_lock;       /*!< spin lock */
    ...
    int (*intr_handler)(struct dma_device_info* info, int status, int chan_no); /*!< DMA pseudo interrupt handler */
...
/* @} */
extern void dma_device_resume_tasklet(void);
extern void dma_device_suspend_tasklet(void);
...
extern int dma_device_read(struct dma_device_info* info, u8** dataptr, void** opt, int chan_no) __attribute__ ((hot));
...
extern int dma_device_write(struct dma_device_info* info, u8* dataptr, int len, void* opt, int chan_no) __attribute__ ((hot));
...
extern int dma_device_desc_setup(_dma_device_info *dma_dev, char *buf, size_t len, int chan_no);
...
*/
extern int dma_device_clear_desc_status_word(_dma_device_info *dma_dev, int dir, int chan_no);
...
extern unsigned long acquire_channel_lock(struct dma_device_info* dma_dev, int tx_chan_no);
extern void free_channel_lock(struct dma_device_info* dma_dev, int tx_chan_no, unsigned long lock_flags);

extern int tx_channels_busy(struct dma_device_info* dma_dev);

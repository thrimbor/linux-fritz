...
#define INT_NUM_IRQ0                    (MIPS_CPU_IRQ_BASE + 2)
 ...
 *
 * Parameter:   IrqNumber
 * Return:      0 continue with interrupt, 1 finish as fast interrupt
\*------------------------------------------------------------------------------------------*/
typedef irqreturn_t (*ifx_low_level_interrupt_t)(int);
extern ifx_low_level_interrupt_t ifx_low_level_interrupt[NR_IRQS];
 
static inline void ifx_register_fast_interrupt(int irq, ifx_low_level_interrupt_t func) {
    ifx_low_level_interrupt[irq] = func;
}
static inline void ifx_release_fast_interrupt(int irq) {
    ifx_low_level_interrupt[irq] = NULL;
}
...
extern void ifx_icu_irsr_set_on(u32 cpu, u32 irq);
extern void ifx_icu_irsr_clr_on(u32 cpu, u32 irq);

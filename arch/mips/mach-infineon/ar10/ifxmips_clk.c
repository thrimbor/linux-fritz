...
static int cgu_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
#else
...
int ltq_cpufreq_adjust_udelay_g = 1;
...
#ifdef CONFIG_IFX_CLOCK_CHANGE
#if defined(ENABLE_SET_CLOCK_PROFILING)
...
static inline void __attribute__((aligned (64))) update_sysclock(u32 cgu_sys __attribute__ ((unused)), u32 cpu_freq)
{
    char name[40];
    ...
    mips_hpt_frequency = ifx_get_cpu_hz() / 2;

    /*--- printk(KERN_ERR "{%s} mips_hpt_freq %d cpu_clk 0x%x\n", __FUNCTION__, mips_hpt_frequency, cgu_sys); ---*/

    clockevent_set_clock(cd, mips_hpt_frequency);

    sprintf(name, "MIPS-%d", cpu_freq);

    /*--- printk(KERN_ERR "{%s} name %s fpi %d\n", __FUNCTION__, name, ifx_get_fpi_hz()); ---*/
    sysfs_override_clocksource(NULL, NULL, name, strlen(name) + 1);

}
#if 0
static void update_sysclock(u32 cgu_sys, u32 cpu_clk)
{
	u64 temp;
	u32 shift;
	...
	mips_hpt_frequency = ifx_get_cpu_hz() / 2;/*half of the CPU frequency*/
	cd = &per_cpu(mips_clockevent_device, smp_processor_id());
	cd->mult = div_sc((unsigned long) mips_hpt_frequency, NSEC_PER_SEC, 32);
	/* END: update clockevent because of frequency change */

	/* START: update clocksource (time) because of frequency change */
	...
	clocksource_unregister(&clocksource_mips);
	clocksource_register(&clocksource_mips);

	/* END: update clocksource because of frequency change */

	/* START: update udelay because of frequency change */
	if(ltq_cpufreq_adjust_udelay_g){
		/* adjust loops_per_jiffy and udelay here */
		loops_per_jiffy = cpufreq_scale(loops_per_jiffy, cpu_freq_old_g, cpu_clk);
		cpu_data[smp_processor_id()].udelay_val = loops_per_jiffy;
	}
	/* END: update udelay because of frequency change */
#else
	mips_hpt_frequency = ifx_get_cpu_hz() / 2;/*half of the CPU frequency*/
    cycles_per_jiffy = (ifx_get_cpu_hz() + HZ) / (HZ * 2);
    /* Calclate a somewhat reasonable rating value */
    clocksource_mips.rating = 200 + mips_hpt_frequency / 10000000;
    /* Find a shift value */
    for ( shift = 32; shift > 0; shift-- )
...
//    register int j;
    ...
    struct { 
        ...
        { .cpu_freq_mhz = 600, .sys_freq_mhz = 300, .param = 0x101 },
        { .cpu_freq_mhz = 600, .sys_freq_mhz = 150, .param = 0x102 },
        { .cpu_freq_mhz = 300, .sys_freq_mhz = 300, .param = 0x111 },
        { .cpu_freq_mhz = 300, .sys_freq_mhz = 150, .param = 0x112 },
        { .cpu_freq_mhz = 150, .sys_freq_mhz = 150, .param = 0x122 },
        { .cpu_freq_mhz = 500, .sys_freq_mhz = 250, .param = 0x001 },
        { .cpu_freq_mhz = 500, .sys_freq_mhz = 125, .param = 0x002 },
        { .cpu_freq_mhz = 250, .sys_freq_mhz = 250, .param = 0x011 },
        { .cpu_freq_mhz = 250, .sys_freq_mhz = 125, .param = 0x012 },
        { .cpu_freq_mhz = 125, .sys_freq_mhz = 250, .param = 0x021 },
        { .cpu_freq_mhz = 125, .sys_freq_mhz = 125, .param = 0x022 }
    };
    u32 cgu_sys;
...
#endif
    int i;
	cpu_freq_old_g = ifx_get_cpu_hz()/1000000;

    for ( i = 0; i < sizeof(cgu_sys_templ); i++ )
        if ( cpu_clk == cgu_sys_templ[i].cpu_freq_mhz && ddr_clk == cgu_sys_templ[i].sys_freq_mhz
            /* && (cgu_sys_templ[i + 2] & 0x18) == (*IFX_CGU_SYS & 0x18) */ )
        {
            cgu_sys = cgu_sys_templ[i].param;
            break;
        }
...
#if defined(ENABLE_ICACHE_LOCK)
    start &= ~(L1_CACHE_BYTES - 1);
    end    = (end + L1_CACHE_BYTES) & ~(L1_CACHE_BYTES - 1) ;
    ifx_fill_lock_icache((void *)start, (end - start));
#else 
...
//    __sync();
    ...
    BARRIER;
    *(volatile u32 *)0xbf8010f0 |= 0x01000001;   //quick refresh + self refresh mode
    BARRIER;

    /* wait until DDR controller acknowledge self refresh */
    while ( (*(volatile u32 *)0xbf8001c0 & 0x2) == 0x2 ); // check CKE bit

    /*  Stop the DDR SDRAM controller by writing a 0 to the START parameter. */
    *(volatile u32 *)0xbf801000 &= ~(0x1);
...
#endif
    while ( (*(volatile u32 *)0xbf8014B0 & 0x1) == 0 );    //check for DLL relock
//    while ( (*(volatile u32 *)0xbf8014E0 & 0x1) == 0 );    //check for DLL relock
...
#endif
    *(volatile u32 *)0xbf8010f0 &= ~(0x1); //Put DDR out of self Refresh mode
//    *(volatile u32 *)0xbf8010f0 |= 0x100; //Power-up Selfrefresh exit

#if !defined(ENABLE_ICACHE_LOCK)
    __asm__("jr     %0"::"r"( end + 16 ));
#endif/*--- #if defined(ENABLE_ICACHE_LOCK) ---*/
    ...
    BARRIER;
#if defined(ENABLE_ICACHE_LOCK)
    ifx_unlock_icache((void *)start, (end - start));
#else /*--- #if defined(ENABLE_ICACHE_LOCK) ---*/
    memcpy((u8*)MBX_BASEADDRESS, (u8*)argv, MPS_MEM_SEG_DATASIZE);
#endif/*--- #else  ---*//*--- #if defined(ENABLE_ICACHE_LOCK) ---*/
...
//	BARRIER;
	...
	update_sysclock(cgu_sys, cpu_clk);

//    local_irq_restore(sys_flag);

#if defined(ENABLE_SET_CLOCK_PROFILING)
    printk(KERN_INFO "Succeeded in switching to CPU %d, DDR %d, FPI %d, clock cycle %u. extra cycle %u\n",
        cpu_clk, ddr_clk, fpi_clk, count_end - count_start, g_extra_counter_end - g_extra_counter_start);
#else
    printk(KERN_INFO "Succeeded in switching to CPU %d, DDR %d, FPI %d\n", cpu_clk, ddr_clk, fpi_clk);
#endif

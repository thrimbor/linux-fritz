...
struct timer_dev_timer {
	...
	unsigned long arg1;
	unsigned long arg2;
	unsigned long original_reload_val;
};
	...
	unsigned long value, unsigned long arg1, unsigned long arg2)
{
	 ...
	 */
	BUG_ON( timer - FIRST_TIMER < 0);

	timer_dev.timer[timer - FIRST_TIMER].flag = flag;
	timer_dev.timer[timer - FIRST_TIMER].arg1 = arg1;
	timer_dev.timer[timer - FIRST_TIMER].arg2 = arg2;
	timer_dev.timer[timer - FIRST_TIMER].original_reload_val = value;
	if (TIMER_FLAG_MASK_SIZE(flag) != TIMER_FLAG_16BIT)
...
unsigned int ifx_gptu_timer_yield_ack( unsigned int timer, unsigned int reload_random_percent) {
	...
	//ackgnoledge IRQ
    __raw_writel(1 << (timer - FIRST_TIMER), IFXMIPS_GPTU_IRNCR);

    if ( reload_random_percent ){

	    static unsigned int p = 0xa690ea64;
        unsigned int random_shift = 0;
        unsigned int flag = timer_dev.timer[timer - FIRST_TIMER].flag;

        if ( reload_random_percent <= 50 )
            random_shift = 1;
        if ( reload_random_percent <= 25 )
            random_shift = 2;
        if ( reload_random_percent <= 13 )
            random_shift = 3;
        if ( reload_random_percent <= 6 )
            random_shift = 4;

        if (TIMER_FLAG_MASK_SIZE(flag) == TIMER_FLAG_16BIT){
    	    short random = (short)( p & 0xFFFF);  // 16bit signed random number
	        random_shift = random_shift + (15 - __fls(timer_dev.timer[timer - FIRST_TIMER].original_reload_val));
    	    random = random >> random_shift;
    	    reload_val = timer_dev.timer[timer - FIRST_TIMER].original_reload_val + random;
    	    if (reload_val & ~0xFFFF)
    	        reload_val = 0xFFFF;
    	} else {
    	    unsigned int random = p;
	        random_shift = random_shift + (31 - __fls(timer_dev.timer[timer - FIRST_TIMER].original_reload_val));
    	    random = random >> random_shift;
    	    reload_val = timer_dev.timer[timer - FIRST_TIMER].original_reload_val + random;
    	}

        *IFXMIPS_GPTU_RELOAD((timer >> 1), (timer & 0x1)) = reload_val;
...
EXPORT_SYMBOL(ifx_gptu_timer_yield_ack);
...
{
	u64 module_freq;
#if defined(CONFIG_DANUBE) || defined(CONFIG_VR9) || defined(CONFIG_AR10) || defined(CONFIG_HN1)

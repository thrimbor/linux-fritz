...
inline u32 ifx_get_ddr_hz(void)
{
    ...
    case 3: return (ifx_get_cpu_hz() * 2) / 5;  //OCP ratio = (vorher 2.5, nach lantiq_patch 0.4)
    ...
    for(i = 0; i < sizeof(clockouttable) / sizeof(clockouttable[0]); i++) {
        if(clk == clockouttable[clkout][i]) {
            ...
            *IFX_CGU_IF_CLK = SET_BITS(*IFX_CGU_IF_CLK, 15 - clkout * 2, 14 - clkout * 2, i);
            ...
            /*--- printk("%s(%d,%d) -> %p:%x\n", __func__, clkout, clk, IFX_CGU_IF_CLK, *IFX_CGU_IF_CLK); ---*/
            return clk;
        }
    }
    return 0;
...
static int proc_write_cgu(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    len = count < sizeof(proc_str) ? count : sizeof(proc_str) - 1;
    rlen = len - copy_from_user(proc_str, buf, len);
    while ( rlen && proc_str[rlen - 1] <= ' ' )
        rlen--;
    proc_str[rlen] = 0;
    for ( p = proc_str; *p && *p <= ' '; p++, rlen-- );
    if ( !*p )
    {
        return 0;
    }
    ...
    if ( stricmp(p, "help") == 0 ) {
        ...
        printk("<cpu clk> 600, 500, 393, 333, 125\n");
        printk("echo clkout > <out> <clk> > /proc/driver/ifx_cgu/clk_setting\n");
    } else if(p == strstr(p, "clkout")) {
        p += sizeof("clkout") - 1;
        ignore_space(&p, &rlen);
        if ( rlen > 0 ) {
            int clkout = get_number(&p, &rlen, 0);
            ignore_space(&p, &rlen);
            if ( rlen > 0 ) {
                int clk = get_number(&p, &rlen, 0);
                cgu_set_clockout(clkout, clk);
            }
        }
    } else {
        ...
        switch ( cpu_clk ) {
            ...
            default:
                ...
                printk("   cpu: 125, ddr: 125\n");
                printk("   cpu: 166, ddr: 166\n");
                printk("   cpu: 333, ddr: 166\n");
                printk("   cpu: 500, ddr: 200\n");
                printk("   cpu: 500, ddr: 250\n");
        }
    }
#endif
...
static inline void __attribute__((aligned (64))) update_sysclock(unsigned int cgu_sys)
{
    char *name = "MIPS\0";
...
u32 __attribute__((aligned (64))) cgu_set_clock(u32 cpu_clk, u32 ddr_clk, u32 fpi_clk) {
    void *start, *end;
...
//                ".set noreorder\n\t"
 ...
 #endif
    // with starting the engine it can happen, that this value is invalid
    if(new_ram_refresh_timing) {
        *(volatile u32 *)0xbf4011e0  = new_ram_refresh_timing; //set new refresh parameter DDR_CCR30
    }
...
static int __init cgu_init(void)
{
    ...
    {
        uint32_t timing = (*(volatile u32 *)IFX_CGU_SYS) & 0xff;
        uint32_t ram_refresh_timing = (*(volatile u32 *)0xbf4011e0) & 0x3fff;

        switch(timing) {
            case 0x80:  // 125, 125, 
                base_ram_refresh_timing = ram_refresh_timing * 1000 / 125;
                break;
            case 0x32:  // 333, 167, 
                base_ram_refresh_timing = ram_refresh_timing * 1000 / 167;
                break;
            case 0x22:  // 393, 196, 
                base_ram_refresh_timing = ram_refresh_timing * 1000 / 196;
                break;
            case 0x13:  // 500, 200, 
                base_ram_refresh_timing = ram_refresh_timing * 1000 / 200;
                break;
            default:
                printk(KERN_ERR "Unknown frequency. Assume 500/250. (*0xbf10300c & 0xff: %u)\n", timing);
            case 0x12:  // 500, 250, 
                base_ram_refresh_timing = ram_refresh_timing * 1000 / 250;
                break;
        }
    }
    //---

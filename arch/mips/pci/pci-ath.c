...
void __init ath_pci_init_board (void) {
    ...
    unsigned int cmd, i;
    int gpio_pcie_reset = 0;

    if(avm_get_hw_config(AVM_HW_CONFIG_VERSION, "gpio_avm_pcie_reset", &gpio_pcie_reset, NULL)) {
        printk("[%s] <no gpio_avm_pcie_reset found>\n", __func__);
    }
	...
	ath_reg_wr_nf(PCIE_PLL_DITHER_DIV_MIN_ADDRESS, PCIE_PLL_DITHER_DIV_MIN_DIV_MIN_INT_SET(0x14));
    ...
    if (gpio_pcie_reset)
        ath_avm_gpio_out_bit(gpio_pcie_reset, 0);
	mdelay(100);
    ...
    if (gpio_pcie_reset)
        ath_avm_gpio_out_bit(gpio_pcie_reset, 1);
	mdelay(100);
	...
	if (((ath_reg_rd(PCIE_RESET_ADDRESS)) & 0x1) == 0x0) {
		printk(KERN_ERR "<Warning : PCIe WLAN Module not found !!!>\n");
	}

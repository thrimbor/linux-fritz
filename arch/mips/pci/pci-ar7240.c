...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void __init ath_pci_init_board (void){
    uint32_t cmd, i;
	unsigned int pcie_pll_div = 5; /*--- MULT ---*/
	unsigned int pcie_pll_revdif = 2;

	/*----------------------------------------------------------------------*\
	 * PCIE aus dem Reset holen 
	\*----------------------------------------------------------------------*/
	ath_reg_rmw_clear(ATH_RESET,ATH_RESET_PCIE_PHY_SHIFT); /*--- reg=0x1806001C  bit 10 ---*/
    mdelay(100);

    ath_reg_rmw_clear(ATH_RESET,ATH_RESET_PCIE_PHY); /*--- reg=0x1806001C  bit 7 ---*/
    ath_reg_rmw_clear(ATH_RESET,ATH_RESET_PCIE);     /*--- reg=0x1806001C  bit 6 ---*/
    ath_reg_wr_nf(ATH_PCI_LCL_RESET, 0);
    mdelay(100);
    ...
    /*--- ath_reg_wr(ATH_PCIE_PLL_CONFIG,0x00040800); ---*/
    mdelay(100);

    ath_reg_wr_nf(ATH_PCI_LCL_RESET, 4);
    mdelay(100);
	...
	for (i = 0; i < ATH_NUM_PCI_CONTROLLER; i++) {
        ath_local_write_config(i, PCI_COMMAND, 4, cmd);
        ath_local_write_config(i, 0x20, 4, 0x1ff01000);
        ath_local_write_config(i, 0x24, 4, 0x1ff01000);
    }

    ath_reg_wr(0x180f0000, 0x1ffc1);
	mdelay(10);
    
	if (((ath_reg_rd(ATH_PCI_LCL_RESET)) & 0x1) == 0x0) {
        printk("[%s]*** Warning *** : PCIe WLAN Module not found !!!\n", __FUNCTION__);
        return;
    }
}
...
#if defined(ATH_LOW_POWER_ENABLE) && ATH_LOW_POWER_ENABLE
...
/*--- #	if !defined(CONFIG_PERICOM) ---*/
	/*--- ath_pci_ops.write(NULL, 0, PCI_COMMAND, 4, ATH_PCI_CMD_INIT); ---*/

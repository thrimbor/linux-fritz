...
static int ifxmips_avm_pcie_chip_reset[IFX_PCIE_CORE_NR];
static int ifxmips_avm_pcie_hw_reset[IFX_PCIE_CORE_NR];
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static void pcie_device_rst_assert(int pcie_port) {
    if(ifxmips_avm_pcie_chip_reset[pcie_port] != -1) {
        if(ifx_gpio_output_clear(ifxmips_avm_pcie_chip_reset[pcie_port], ifx_pcie_gpio_module_id) == IFX_ERROR) {
            printk(KERN_ERR "[%s] ifxmips_avm_pcie_chip_reset[%d] reset fsiled \n", __FUNCTION__, pcie_port);
        } else {
            printk("[%s] ifxmips_avm_pcie_chip_reset[%d] reseted \n", __FUNCTION__, pcie_port);
        }
    }
    if(ifxmips_avm_pcie_hw_reset[pcie_port] != -1) {
        if(ifx_gpio_output_clear(ifxmips_avm_pcie_hw_reset[pcie_port], ifx_pcie_gpio_module_id) == IFX_ERROR) {
            printk(KERN_ERR "[%s] ifxmips_avm_pcie_hw_reset[%d] reset fsiled \n", __FUNCTION__, pcie_port);
        } else {
            printk("[%s] ifxmips_avm_pcie_hw_reset[%d] reseted \n", __FUNCTION__, pcie_port);
        }
    }
}
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static void pcie_device_rst_deassert(int pcie_port) {
    printk("[%s] ifxmips_avm_pcie_chip_reset  set\n", __FUNCTION__);
    if(ifxmips_avm_pcie_chip_reset[pcie_port] != -1) {
        if(ifx_gpio_output_set(ifxmips_avm_pcie_chip_reset[pcie_port], ifx_pcie_gpio_module_id) == IFX_ERROR) {
            printk(KERN_ERR "[%s] ifxmips_avm_pcie_chip_reset[%d] release failed\n", __FUNCTION__, pcie_port);
        } else {
            printk("[%s] ifxmips_avm_pcie_chip_reset[%d] released \n", __FUNCTION__, pcie_port);
        }
    }
    mdelay(100);
    if(ifxmips_avm_pcie_hw_reset[pcie_port] != -1) {
        if(ifx_gpio_output_set(ifxmips_avm_pcie_hw_reset[pcie_port], ifx_pcie_gpio_module_id) == IFX_ERROR) {
            printk(KERN_ERR "[%s] ifxmips_avm_pcie_hw_reset[%d] release failed\n", __FUNCTION__, pcie_port);
        } else {
            printk("[%s] ifxmips_avm_pcie_hw_reset[%d] released \n", __FUNCTION__, pcie_port);
        }
    }
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static void pcie_ep_rst_init(int pcie_port) {
    char name[80];
    int ret;

    ret = ifx_gpio_register(ifx_pcie_gpio_module_id);
    if(ret == IFX_ERROR) {
        printk(KERN_ERR "[%s] Error: Registering GPIO module ID %d failed!\n", __FUNCTION__, ifx_pcie_gpio_module_id);
        return;
    }

    if(pcie_port > 0) {
        sprintf(name, "gpio_avm_pcie_reset%d", pcie_port);
    } else {
        strcpy(name, "gpio_avm_pcie_reset");
    }

    if(avm_get_hw_config(AVM_HW_CONFIG_VERSION, name, &ifxmips_avm_pcie_hw_reset[pcie_port], NULL)) {
        ifxmips_avm_pcie_hw_reset[pcie_port] = -1;
    } else {
        printk("[%s] %s[%d] = GPIO %d\n", __FUNCTION__, name, pcie_port, ifxmips_avm_pcie_hw_reset[pcie_port]);
    }

    if(pcie_port > 0) {
        sprintf(name, "gpio_avm_pcie_chip_reset%d", pcie_port);
    } else {
        strcpy(name, "gpio_avm_pcie_chip_reset");
    }
    if(avm_get_hw_config(AVM_HW_CONFIG_VERSION, name, &ifxmips_avm_pcie_chip_reset[pcie_port], NULL)) {
        ifxmips_avm_pcie_chip_reset[pcie_port] = -1;
    } else {
        printk("[%s] %s[%d] = GPIO %d\n", __FUNCTION__, name, pcie_port, ifxmips_avm_pcie_chip_reset[pcie_port]);
    }
...
#define IFX_PCIE_PHY_LOOP_CNT  5
    ...
    if(avm_get_hw_config(AVM_HW_CONFIG_VERSION, 
                pcie_port == 0 ? "pcie_disable_interface_0" : "pcie_disable_interface_1",
    ...
    for (i = 0; i < IFX_PCIE_PHY_LOOP_CNT; i++) {
        ...
        if(disable_interface)  /* interface ist abgeschaltet */
            return -EIO;

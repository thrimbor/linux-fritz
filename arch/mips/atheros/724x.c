...
#define         UART16550_WRITE(x, z)  UART_WRITE(x, z)
extern unsigned int serial_inited;
...
#endif /*--- #ifdef CONFIG_USB_OHCI_ATH ---*/
...
#endif /*--- #ifdef CONFIG_USB_EHCI_AR9130 ---*/
...
#endif /*--- #ifdef CONFIG_USB_SUPPORT ---*/ 
...
int ath_platform_init(void)
{
    ...
    printk("===== ath_platform_init: %d\n", ret);
	if (ret < 0) {
		printk("ath_platform_init: uart failed %d\n", ret);
		return ret;
    }
#endif
    printk("[%s] init devices for '%s'\n", __FUNCTION__, CONFIG_ATH_SYS_TYPE);
	...
	if (ret < 0) {
		printk("ath_platform_init: usb failed %d\n", ret);
		return ret;
	}
#endif /*--- #ifdef CONFIG_USB_SUPPORT ---*/ 

...
unsigned int ar7240_slic_cntrl_rd(void)  { return ar7240_reg_rd(AR7240_SLIC_CTRL);  }
void ar7240_slic_cntrl_wr(unsigned int val)       { ar7240_reg_wr(  AR7240_SLIC_CTRL, val);    }
#else
...
ar7240_spi_raw_output_u8(unsigned char val)
{
  ...
  for (ii = 7; ii>=0 ; ii--) {
    ...
    for (ii = 0; ii<8 ; ii++) {
        dac_data = 0x50000; //AR7240_SPI_CS_ENABLE_1;
        if ( (reg>>(7-ii) & 0x1) == 0x1) {//AR7240_SPI_D0_HIGH) == AR7240_SPI_D0_HIGH)
          dac_data = dac_data | 0x1; //AR7240_SPI_D0_HIGH;
        }
        else
        {
          dac_data = dac_data & 0xfffffffe; //~(AR7240_SPI_D0_HIGH);
        }
        ar7240_reg_wr (AR7240_SPI_WRITE,dac_data);
    
        dac_data = dac_data | 0x50100; //(AR7240_SPI_CS_ENABLE_1|AR7240_SPI_CLK_HIGH);
        ar7240_reg_wr (AR7240_SPI_WRITE, dac_data);
    }
#else
   ...
   for (i=0; i<8; i++)   // TRANSMIT DATA
   {
       ...
       ar7240_reg_wr (0xbf000008,dac_data);
...
ar7240_spi_raw_input_u8(void) 
{
  ...
  return ar7240_reg_rd(AR7240_SPI_RD_STATUS) & 0xff;
    ...
    unsigned int ii;
    for (ii = 7; ii>=0 ; ii--) {
        ar7240_reg_wr(AR7240_SPI_WRITE, 0x50100); //(AR7240_SPI_CS_ENABLE_1|AR7240_SPI_CLK_HIGH));
        ar7240_reg_wr(AR7240_SPI_WRITE, 0x50000); //AR7240_SPI_CS_ENABLE_1);   //CS1 = 0 , CLK=0
    }
    ar7240_reg_wr(AR7240_SPI_WRITE, 0x70000);       //CS1 = 1 , CLK=0
   ...
   ar7240_reg_wr(0xbf000008, 0x50100); //CS1 = 0 , CLK=1
   for (j=0;j<15;j++) { ; };

   ar7240_reg_wr(0xbf000008, 0x50000); //CS1 = 0 , CLK=0
   for (j=0;j<15;j++) { ; };
    }
   ar7240_reg_wr(0xbf000008, 0x70000); //CS1 = 1 , CLK=0
   for (j=0;j<15;j++) { ; };

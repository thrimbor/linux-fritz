...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static void  __init ar7240_init_cmdline(int argc, char *argv[]) {
	char *cp;
	int actr;
...
void __init prom_init(void) {
    ...
    serial_print("[%s]:\n", __FUNCTION__);
	ar7240_init_cmdline(fw_arg0, (char **)fw_arg1);
...
/*--------------------------------------------------------------------------------*\
\*--------------------------------------------------------------------------------*/
extern struct mtd_info *ar7240_urlader_mtd;
...
static int wlan_dect_read_config(int offset, unsigned int len, unsigned char *buffer) {
    ...
    ar7240_urlader_mtd->read(  ar7240_urlader_mtd,
                                offset & ~1, 
...
int get_wlan_dect_config(enum wlan_dect_type Type, unsigned char *buffer, unsigned short len) {
    ...
    for (i=0;i<XR9_MAX_CONFIG_ENTRIES;i++) {
        ...
        if (wlan_dect_config[i]) {    /*--- Eintrag vorhanden und nicht leer ---*/
            ...
            ar7240_urlader_mtd->read(ar7240_urlader_mtd, offset & ~1, 2 * sizeof(struct wlan_dect_config), &readlen, tmpbuffer);

...
static struct mtd_partition ar7240_partitions[MAX_FLASH_MTD];
/*-------------------------------------------------------------------------------------*\
...
static struct ar7240_flash_data _ar7240_flash_data = {
	.flash_size	= 16, /*--- __ath_flash_size ---*/
	.parts		= ar7240_partitions,
	.nr_parts	= ARRAY_SIZE(ar7240_partitions),
    .probes     = probes
};

/* NOTE: CFI probe will correctly detect flash part as 32M, but EMIF
 * limits addresses to 16M, so using addresses past 16M will wrap */
static struct resource ar7240_flash_resource[2] = {
    {
        .start		= 0x1f000000,
...
void ar7240_ram_mtd_set_rw(struct device *pdev, int);

static int use_ar7240_flash_device = 0;
struct platform_device ar7240_flash_device = {
	.name		= "ar7240-nor",
	.id		    = 0,
	.dev		= {
		.platform_data	= &_ar7240_flash_data,
	},
	.num_resources	= 1,
	.resource	= &ar7240_flash_resource[0],
};


/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static struct mtd_partition ar7240_ram_partitions[3];

static int use_ar7240_ram_device = 0;
static struct platdata_mtd_ram ar7240_ram_data = {
	.mapname       = "ram-filesystem",
	.bankwidth	   = 4,
	.partitions    = ar7240_ram_partitions,
	.nr_partitions = 0, /*--- ARRAY_SIZE(ar7240_ram_partitions), ---*/
    .set_rw        = ar7240_ram_mtd_set_rw,
    .probes        = probes
};

struct platform_device ar7240_ram_device = {
	.name		= "mtd-ram",
	.id		    = -1,
	.dev		= {
		.platform_data	= &ar7240_ram_data,
	},
	.num_resources	= 1,
	.resource	= &ar7240_flash_resource[1],
};


/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void ar7240_ram_mtd_set_rw(struct device *pdev, int mode) {
    if(mode == PLATRAM_RO) {
        printk(KERN_ERR "[ar7240_ram_mtd_set_rw] PLATRAM_RO\n");
    } else if(mode == PLATRAM_RW) {
        printk(KERN_ERR "[ar7240_ram_mtd_set_rw] PLATRAM_RW\n");
    }
}
...
static int ar7240_squashfs_parser_function(struct mtd_info *mtd, struct mtd_partition **p_mtd_pat, unsigned long param) {
    ...
    if (!strcmp(mtd->name, "ram-filesystem")) {
        maptype = MAP_RAM;
    } else if (!strcmp(mtd->name, "ar7240-nor")) {
        maptype = MAP_FLASH;
    ...
    if(p_mtd_pat) {
        ...
        switch (maptype) {
            case MAP_FLASH:
                if(*p_mtd_pat == NULL) {
                    *p_mtd_pat = ar7240_partitions;
                }
                maxcount = ARRAY_SIZE(ar7240_partitions);
                break;
            case MAP_RAM:
                if(*p_mtd_pat == NULL) {
                    *p_mtd_pat = ar7240_ram_partitions;
                }
                maxcount = ARRAY_SIZE(ar7240_ram_partitions);
                break;
            default:
                break;
        }
        ...
        while( pos < end_offset ) {
			...
			if(magic == 0x73717368 || DUMMY_MAGIC(pos, end_offset)) {
                ...
                if ((maptype == MAP_FLASH) && (memcmp(ar7240_partitions[5].name, "jffs2", 4) != 0)) {
                    ...
                    if (jffs2_size < (JFFS2_MIN_SIZE * (mtd->erasesize/0x10000))) {
                        ...
                        ar7240_partitions[5].offset = jffs2_start;
                        ar7240_partitions[5].size   = jffs2_size * 0x10000;
                        ar7240_partitions[5].name   = "jffs2";
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static int ar7240_jffs2_parser_function(struct mtd_info *mtd, struct mtd_partition **p_mtd_pat, unsigned long param) {
    enum _flash_map_enum maptype = MAP_UNKNOWN;
    unsigned count = 1, maxcount = 0;
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
struct mtd_part_parser ar7240_squashfs_parser = {
	.name     = "find_squashfs",
	.parse_fn = ar7240_squashfs_parser_function
};

/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
struct mtd_part_parser ar7240_jffs2_parser = {
	.name     = "find_jffs2",
	.parse_fn = ar7240_jffs2_parser_function
};
...
struct mtd_info * ar7240_urlader_mtd;

void __init ar7240_mtd_add_notifier(struct mtd_info *mtd) {
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void __init ar7240_mtd_rm_notifier(struct mtd_info *mtd) {
    printk("[ar7240_mtd_rm_notifier] ignore %s\n", mtd->name);
}

struct mtd_notifier __initdata ar7240_mtd_notifier = {
    add: ar7240_mtd_add_notifier,
    remove: ar7240_mtd_rm_notifier
};
...
int __init ar7240_mtd_init(void) {

	printk("[%s]\n", __FUNCTION__);
    /*--- mtdflash_setup(prom_getenv((char*)"flashsize")); ---*/
    register_mtd_parser(&ar7240_squashfs_parser);
    register_mtd_parser(&ar7240_jffs2_parser);
    register_mtd_user(&ar7240_mtd_notifier);

	if ( use_ar7240_flash_device ) {
		printk("[%s] flash: type=%ld, start=%#x, end=%#x \n", __FUNCTION__, ar7240_flash_device.resource->flags, ar7240_flash_device.resource->start,  ar7240_flash_device.resource->end );
		platform_device_register( &ar7240_flash_device );
	}

	if ( use_ar7240_ram_device ) {
		printk("[%s] ram: type=%ld, start=%#x, end=%#x \n", __FUNCTION__, ar7240_ram_device.resource->flags, ar7240_ram_device.resource->start,  ar7240_ram_device.resource->end );
		platform_device_register( &ar7240_ram_device );
	} 
    return 0;
}
subsys_initcall(ar7240_mtd_init);
...
static int __init mtdflash_setup(char *p) {
    ...
    ar7240_partitions[0].name = (char *)"filesystem";
    ar7240_partitions[1].name = (char *)"kernel";
    ar7240_partitions[2].name = (char *)"urlader";
    ar7240_partitions[3].name = (char *)"tffs (1)";
    ar7240_partitions[4].name = (char *)"tffs (2)";
    ar7240_partitions[5].name = (char *)"reserved";
    ...
    if(p) {
        ...
        if(p) {
            ...
            ar7240_partitions[2].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[2].offset     = mtd_start - flashoffset;
            ar7240_flash_resource[0].start = mtd_start;
            ar7240_flash_resource[0].end   = mtd_start + flashsize;
        }
    }
    p = prom_getenv("mtd0");
    if(p) {
        ...
        if(p) {
            p++;
            mtd_end  = CPHYSADDR((unsigned int)simple_strtoul(p, NULL, 16) - 1);
            ar7240_partitions[0].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[0].offset     = mtd_start - flashoffset;
        }
    }
    p = prom_getenv("mtd1");
    if(p) {
        ...
        if(p) {
            p++;
            mtd_end  = CPHYSADDR((unsigned int)simple_strtoul(p, NULL, 16) - 1);
            ar7240_partitions[1].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[1].offset     = mtd_start - flashoffset;
        }
    }
    p = prom_getenv("mtd3");
    if(p) {
        ...
        if(p) {
            p++;
            mtd_end  = CPHYSADDR((unsigned int)simple_strtoul(p, NULL, 16) - 1);
            ar7240_partitions[3].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[3].offset     = mtd_start - flashoffset;
        }
    }
    p = prom_getenv("mtd4");
    if(p) {
        ...
        if(p) {
            p++;
            mtd_end  = CPHYSADDR((unsigned int)simple_strtoul(p, NULL, 16) - 1);
            ar7240_partitions[4].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[4].offset     = mtd_start - flashoffset;
        }
    }
    p = prom_getenv("mtd5");
    if(p) {
        ...
        if(p) {
            ...
            ar7240_partitions[5].size       = mtd_end - mtd_start + 1;
            ar7240_partitions[5].offset     = mtd_start - flashoffset;
        }
    }
	use_ar7240_flash_device = 1;
    return 0;
}
...
static int __init mtdram_setup(char *p) {
    ...
    if(p) {
        ...
        ar7240_flash_resource[1].start  = my_atoi(p);
        ar7240_flash_resource[1].start &= ~0xE0000000;
        ar7240_flash_resource[1].flags  = IORESOURCE_MEM,
        p = strchr(p, ',');
        if(p) {
            p++;
            ar7240_flash_resource[1].end  = my_atoi(p);
            ar7240_flash_resource[1].end &= ~0xE0000000;
            ar7240_flash_resource[1].end -= 1;
        } else {
            ar7240_flash_resource[1].start = 0;
        }
        DEBUG_MTD("[%s] mtdram1 0x%08x - 0x%08x\n", __FUNCTION__, ar7240_flash_resource[1].start, ar7240_flash_resource[1].end );
        ar7240_ram_partitions[0].name		 = "rootfs_ram";
        ar7240_ram_partitions[0].offset	 = 0;
        ar7240_ram_partitions[0].size		 = ar7240_flash_resource[1].end - ar7240_flash_resource[1].start + 1;
        ar7240_ram_partitions[0].mask_flags = MTD_ROM;
        ar7240_ram_partitions[1].name		 = "unused";
        ar7240_ram_partitions[1].offset	 = 0;
        ar7240_ram_partitions[1].size		 = ar7240_flash_resource[1].end - ar7240_flash_resource[1].start + 1;
        ar7240_ram_partitions[1].mask_flags = MTD_ROM;
        ar7240_ram_partitions[2].name		 = "extra";
        ar7240_ram_partitions[2].offset	 = 0;
        ar7240_ram_partitions[2].size		 = ar7240_flash_resource[1].end - ar7240_flash_resource[1].start + 1;
        ar7240_ram_partitions[2].mask_flags = MTD_ROM;
    }
	use_ar7240_ram_device = 1;
    return 0;
}

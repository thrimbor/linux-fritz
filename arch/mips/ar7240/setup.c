...
uint32_t ar7240_cpu_freq, ar7240_ahb_freq, ar7240_ddr_freq;
#ifdef CONFIG_WASP_SUPPORT
...
printk(KERN_ERR "[%s] out_div 0x%d\n", __FUNCTION__, out_div);
	ref_div	= CPU_PLL_CONFIG_REFDIV_GET(pll);
printk(KERN_ERR "[%s] ref_div 0x%d\n", __FUNCTION__, ref_div);
	nint	= CPU_PLL_CONFIG_NINT_GET(pll);
printk(KERN_ERR "[%s] nint 0x%d\n", __FUNCTION__, nint);
	frac	= CPU_PLL_CONFIG_NFRAC_GET(pll);
printk(KERN_ERR "[%s] frac 0x%d\n", __FUNCTION__, frac);
	ar7240_cpu_freq = ((nint * ref / ref_div) >> out_div) /
			(CPU_DDR_CLOCK_CONTROL_CPU_POST_DIV_GET(clk_ctrl) + 1);
printk(KERN_ERR "[%s] freq 0x%d\n", __FUNCTION__, ar7240_cpu_freq );
...
ar7240_sys_frequency(void)
{
	...
	printk("[%s] pll_div=%d, ref_div=%d, ddr_div=%d, ahb_div=%d\n", __FUNCTION__, pll_div, ref_div, ddr_div, ahb_div);
	printk("[%s] cpu_freq=%d, ddr_freq=%d, ahb_freq=%d \n", __FUNCTION__, ar7240_cpu_freq, ar7240_ddr_freq, ar7240_ahb_freq);
#endif
#endif /* CONFIG_MACH_HORNET */
}
#endif
#ifdef CONFIG_SERIAL_8250
void __init ar7240_serial_setup(void) {
	struct uart_port p;
	...
	p.regshift  = 2;
	p.mapbase   = (u32)KSEG1ADDR(AR7240_UART_BASE);
	p.membase   = (void __iomem *)p.mapbase;

	if (early_serial_setup(&p) != 0) {
		serial_print("early_serial_setup failed\n");
        return 0;
    }
...
unsigned int ar7240_serial_in(int offset) {
            return UART16550_READ(offset);
}

void ar7240_serial_out(int offset, int value) {
...
static ssize_t ar7240_ioc_read(struct file * file, char * buf, size_t count, loff_t *ppos) {
 ...
 */
static int __init ar7240_init_ioc() {
    static int _mymajor;

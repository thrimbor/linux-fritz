...
#define FLAG_PPE_CLK_CHANGE_ARG_AVM_DIRCT (1 << 1)
...
#define SESSION_BYTE_STAMPING                   0x00000008 //for PPA session management purse
#define SESSION_ADDED_IN_HW                     0x00000010
#define SESSION_NON_ACCE_MASK   ~SESSION_ADDED_IN_HW    //for ioctl only
#define SESSION_CAN_NOT_ACCEL                   0x00000020  // later may need to change the naming, so far it also used for sesson management
...
#define SESSION_NOT_ACCELABLE                  0x00000800  //Session cannot be accelerated at all
...
#define SESSION_VALID_AVM_PORTMAP               0x00800000
...
#define WRAPROUND_24BITS  0xFFFFFF /*note, this value will be different with GRX500 platform */

#define SESSION_FLAGS_ARRAY static const char *session_flags_array[] = { \
        "IS_REPLY",                  \
        "BR_VCI_CHECK",        \
        "IS_TCP",            \
        "STAMPING",                  \
...
typedef struct {
    ...
    uint32_t                  collision_flag;            /*!< 1 mean the entry is in collsion table or none-hashed, like ASE/Danubel*/
...
typedef struct {
    ...
    uint32_t mib_count;  //for MIB
    uint32_t mib_max;    //here will be wrapped: mib_max + 1 = 0
    uint32_t mib_factor; //byte_count = mib_count * mib_factor

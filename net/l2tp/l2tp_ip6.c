...
pass_up:
	...
	if (sk == NULL || sock_flag(sk, SOCK_DEAD))
		goto discard;
...
static int l2tp_ip6_bind(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
	...
	err = -EADDRINUSE;
	read_lock_bh(&l2tp_ip6_lock);
	if (__l2tp_ip6_bind_lookup(sock_net(sk), &addr->l2tp_addr,
			    ...
			    struct msghdr *msg, size_t len)
{
	...
	if (lsa) {
		...
		daddr = &lsa->l2tp_addr;
		if (np->sndflow) {
			fl6.fl6_flowlabel = lsa->l2tp_flowinfo & IPV6_FLOWINFO_MASK;
			if (fl6.fl6_flowlabel&IPV6_FLOWLABEL_MASK) {
				flowlabel = fl6_sock_lookup(sk, fl6.fl6_flowlabel);
				if (flowlabel == NULL)
					return -EINVAL;
				daddr = &flowlabel->dst;
			}
		...
		if (addr_len >= sizeof(struct sockaddr_in6) &&
			...
			fl6.oif = lsa->l2tp_scope_id;
	} else {
		if (sk->sk_state != TCP_ESTABLISHED)
			return -EDESTADDRREQ;

		daddr = &np->daddr;
		fl6.fl6_flowlabel = np->flow_label;
	}

	if (fl6.oif == 0)
		fl6.oif = sk->sk_bound_dev_if;
	...
	if (msg->msg_controllen) {
		...
		err = datagram_send_ctl(sock_net(sk), msg, &fl6, opt,
					    &hlimit, &tclass);
		if (err < 0) {
			fl6_sock_release(flowlabel);
			return err;
		}
		if ((fl6.fl6_flowlabel & IPV6_FLOWLABEL_MASK) && !flowlabel) {
			flowlabel = fl6_sock_lookup(sk, fl6.fl6_flowlabel);
			if (flowlabel == NULL)
				return -EINVAL;
		}
	...
	fl6.proto = sk->sk_protocol;
	if (!ipv6_addr_any(daddr))
		fl6.fl6_dst = *daddr;
	else
		fl6.fl6_dst.s6_addr[15] = 0x1; /* :: means loopback (BSD'ism) */
	if (ipv6_addr_any(&fl6.fl6_src) && !ipv6_addr_any(&np->saddr))
		fl6.fl6_src = np->saddr;

	if (!fl6.oif && ipv6_addr_is_multicast(&fl6.fl6_dst))
		fl6.oif = np->mcast_oif;
	...
	if (flowlabel) {
		if (flowlabel->opt && flowlabel->opt->srcrt) {
			...
			ipv6_addr_copy(&final, &fl6.fl6_dst);
			ipv6_addr_copy(&fl6.fl6_dst, rt0->addr);
			final_p = &final;
		}
	...
	if (err)
		goto out;
	if (final_p)
		ipv6_addr_copy(&fl6.fl6_dst, final_p);

	err = __xfrm_lookup(sock_net(sk), &dst, &fl6, sk, XFRM_LOOKUP_WAIT);
	if (err < 0) {
		if (err == -EREMOTE)
			err = ip6_dst_blackhole(sk, &dst, &fl6);
		if (err < 0)
			goto out;
	}

	if (hlimit < 0) {
		if (ipv6_addr_is_multicast(&fl6.fl6_dst))
